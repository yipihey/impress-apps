...
#[derive(Clone)]
pub struct DefaultImbibUndoService {
...
fn log(m: &str, e: impl std::fmt::Display) {
    eprintln!("[imbib-undo-service] {m}: {e}");
}
...
impl ImbibUndoService for DefaultImbibUndoService {
    async fn recent_undo_groups(&self, max_entries: u32) -> Vec<UndoGroupRecord> {
        ...
        self.store
            .recent_undo_groups(n)
            .map(|rs| rs.iter().map(UndoGroupRecord::from).collect::<Vec<_>>())
            .unwrap_or_else(|e| {
                log("recent_undo_groups", e);
                vec![]
            })
    }
    async fn undo_operation(&self, operation_id: String) -> MutationResult {
        match self.store.undo_operation(operation_id) {
            Ok(info) => MutationResult {
                affected_count: info.operation_ids.len() as u32,
                ok: true,
            },
            Err(e) => {
                log("undo_operation", e);
                MutationResult {
                    affected_count: 0,
                    ok: false,
                }
