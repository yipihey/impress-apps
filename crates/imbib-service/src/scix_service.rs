...
#[derive(Clone)]
pub struct DefaultImbibScixService { store: Arc<ImbibStore> }
impl DefaultImbibScixService { pub fn new(store: Arc<ImbibStore>) -> Self { Self { store } } }
...
fn log(m: &str, e: impl std::fmt::Display) { eprintln!("[imbib-scix-service] {m}: {e}"); }

#[async_trait::async_trait]
impl ImbibScixService for DefaultImbibScixService {
    async fn list_scix_libraries(&self) -> Vec<SciXLibraryRecord> {
        self.store.list_scix_libraries()
            .map(|rs| rs.iter().map(SciXLibraryRecord::from).collect::<Vec<_>>())
            .unwrap_or_else(|e| { log("list_scix_libraries", e); vec![] })
    }
    async fn get_scix_library(&self, id: String) -> Option<SciXLibraryRecord> {
        self.store.get_scix_library(id).ok().flatten().as_ref().map(SciXLibraryRecord::from)
    }
    async fn create_scix_library(&self, remote_id: String, name: String, description: Option<String>, is_public: bool, permission_level: String, owner_email: Option<String>) -> Option<SciXLibraryRecord> {
        self.store.create_scix_library(remote_id, name, description, is_public, permission_level, owner_email)
    ...
    async fn add_to_scix_library(&self, publication_ids: Vec<String>, scix_library_id: String) -> MutationResult {
        ...
        match self.store.add_to_scix_library(publication_ids, scix_library_id) {
            Ok(_) => ok_n(n), Err(e) => { log("add_to_scix_library", e); fail() }
        }
    }
    async fn remove_from_scix_library(&self, publication_ids: Vec<String>, scix_library_id: String) -> MutationResult {
        let n = publication_ids.len() as u32;
        match self.store.remove_from_scix_library(publication_ids, scix_library_id) {
            Ok(_) => ok_n(n), Err(e) => { log("remove_from_scix_library", e); fail() }
        }
    }
    async fn query_scix_library_publications(&self, scix_library_id: String, sort_field: String, ascending: bool, limit: u32, offset: u32) -> Vec<PublicationSummary> {
        ...
        self.store.query_scix_library_publications(scix_library_id, sort, ascending, lim, off)
            .map(|rs| rs.iter().map(PublicationSummary::from).collect::<Vec<_>>())
            .unwrap_or_else(|e| { log("query_scix_library_publications", e); vec![] })
    }
    async fn count_scix_library_publications(&self, scix_library_id: String) -> u32 {
        self.store.count_scix_library_publications(scix_library_id).unwrap_or(0)
    }
}
