...
#[derive(Clone)]
pub struct DefaultImbibArtifactsService {
...
fn log(m: &str, e: impl std::fmt::Display) {
    eprintln!("[imbib-artifacts-service] {m}: {e}");
}
...
impl ImbibArtifactsService for DefaultImbibArtifactsService {
    async fn list_artifacts(
        ...
        self.store
            .list_artifacts(schema_filter, sort, ascending, lim, off)
            .map(|rs| rs.iter().map(ArtifactRecord::from).collect::<Vec<_>>())
            .unwrap_or_else(|e| {
                log("list_artifacts", e);
                vec![]
            })
    }
    async fn search_artifacts(
    ...
    async fn update_artifact(
        ...
        match self.store.update_artifact(
            id,
            title,
            source_url,
            notes,
            ...
            Ok(_) => ok_n(1),
            Err(e) => {
                log("update_artifact", e);
                fail()
            }
        }
    }
    async fn delete_artifact(&self, id: String) -> MutationResult {
        match self.store.delete_artifact(id) {
            Ok(_) => ok_n(1),
            Err(e) => {
                log("delete_artifact", e);
                fail()
            }
        }
    }
    async fn link_artifact_to_publication(
            ...
            .link_artifact_to_publication(artifact_id, publication_id)
        {
            Ok(_) => ok_n(1),
            Err(e) => {
                log("link_artifact_to_publication", e);
                fail()
            }
        }
    }
    async fn get_artifact_relations(&self, id: String) -> Vec<ArtifactRelationRecord> {
        self.store
            ...
            .map(|rs| {
                rs.iter()
                    .map(ArtifactRelationRecord::from)
                    .collect::<Vec<_>>()
            })
            .unwrap_or_else(|e| {
                log("get_artifact_relations", e);
                vec![]
            })
    }
}
