...
//! and writes MCP server configuration so they can use im-bibtex tools.
...
fn detect_editors(filter: Option<&EditorTarget>) -> Vec<EditorInfo> {
    ...
    for (target, config_path, detect_dir) in candidates {
        ...
        // Editor is "detected" if its config dir exists or config file exists
...
const SERVER_NAME: &str = "im-bibtex";
