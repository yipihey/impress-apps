...
fn compiles_to_pdf_with_synctex() {
    ...
    assert!(pdf.len() > 500, "PDF suspiciously small: {} bytes", pdf.len());
    assert_eq!(&pdf[..5], b"%PDF-", "output does not start with %PDF-");
    assert!(r.synctex_data.is_some(), "SyncTeX data was requested but not produced");
    eprintln!(
...
fn reports_error_on_broken_source() {
    ...
    assert!(r.error.is_some(), "expected an error summary for broken source");
...
fn resolves_on_disk_input_via_filesystem_root() {
    ...
    std::fs::write(dir.path().join("chapter.tex"), r"Included chapter body.")
        .expect("write aux");
    ...
    assert!(without.pdf_data.is_none(), "expected failure without filesystem_root");

    // With filesystem_root → resolves the on-disk file → PDF.
    let with = compile_latex_tectonic(src, false, None, dir.path().to_str());
    assert!(with.error.is_none(), "unexpected error with root: {:?}", with.error);
    assert!(with.pdf_data.is_some(), "expected a PDF once filesystem_root is set");
    eprintln!(
