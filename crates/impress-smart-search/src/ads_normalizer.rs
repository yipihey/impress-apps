...
//!
//! Port of Swift `ADSQueryNormalizer.swift`. Relocated here from
//! `imbib-core::search::ads_normalizer` during the smart-search port: that
//! module was a complete but *unwired* Rust twin — nothing but a property test
//! referenced it, while two byte-identical Swift copies (one in
//! `ImpressSmartSearch`, one in `PublicationManagerCore`) did the real work.
//! Three implementations of one rule set is two too many, so this is now the
//! only one and `imbib-core` re-exports it. Pure-string transformation that
...
use crate::types::NormalizationResult;
...
fn expand_shorthands(query: &str) -> (String, Vec<String>) {
    ...
    for (short, full) in SHORTHANDS.iter() {
        ...
        // scan.
        // Swift iterates `matches.reversed()`, so the corrections for one
        // shorthand come out last-match-first.
        let mut per_shorthand = Vec::new();
        result = expand_one_shorthand(&result, short, full, &mut per_shorthand);
        per_shorthand.reverse();
        corrections.extend(per_shorthand);
    }
...
fn expand_one_shorthand(
    ...
    while i < bytes.len() {
        ...
        if starts_here {
            ...
            let next_is_non_space = next_idx < bytes.len()
                && !input[next_idx..]
                    .chars()
                    .next()
...
fn remove_spaces_after_colons(query: &str) -> (String, Vec<String>) {
    ...
    corrections.reverse(); // Swift iterates `matches.reversed()`.
    (out, corrections)
}
...
lazy_static! {
    /// Swift: `(?<!["\w])\b(and|or|not)\b(?!["\w])`, case-insensitive.
    ///
    /// The `regex` crate has no lookaround, and the obvious workaround —
    /// folding the guards into *consuming* character classes,
    /// `(?:^|[^"\w])(and|or|not)(?:$|[^"\w])` — is subtly wrong: it eats the
    /// delimiter, so in `"and or not"` the space after `and` is consumed and
    /// `or` can no longer find a preceding non-word character. That produced
    /// `"AND or NOT"`. This is exactly the class of bug the task flagged as a
    /// known trap, and the golden corpus caught it.
    ///
    /// Instead: match the operator with `\b` only (zero-width, so nothing is
    /// consumed) and check the neighbouring characters by hand. `\b` already
    /// guarantees the neighbours are non-word, so only the `"` exclusion is
    /// left to test.
    static ref BOOL_OP_RE: Regex = Regex::new(r#"(?i)\b(and|or|not)\b"#).unwrap();
}
...
fn uppercase_boolean_operators(query: &str) -> (String, Vec<String>) {
    ...
    for cap in BOOL_OP_RE.captures_iter(query) {
        ...
        // Zero-width guards: an immediately adjacent double quote blocks the
        // rewrite. Note this does NOT mean "inside quotes" — the Swift comment
        // claims that, but `abs:"dark and matter"` does get uppercased, because
        // the characters next to `and` are spaces. Behavior preserved.
        let prev_is_quote = query[..g.start()].ends_with('"');
        let next_is_quote = query[g.end()..].starts_with('"');
