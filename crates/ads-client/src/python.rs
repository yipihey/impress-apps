//! Python bindings for the ads-client crate.
//!
...
//!
//! Only `AdsClient` (async→sync bridge) and `QueryBuilder` (ownership→mutation bridge)
...
/// Convert an AdsError into a Python exception.
fn to_py_err(e: AdsError) -> PyErr {
...
// ---------------------------------------------------------------------------
// PyAdsClient — synchronous wrapper around the async AdsClient
// ---------------------------------------------------------------------------

/// NASA ADS / SciX API client.
///
/// Provides synchronous access to all ADS endpoints.
///
/// Example:
///     client = ads_client.AdsClient()          # reads ADS_API_TOKEN env var
///     client = ads_client.AdsClient("token")   # explicit token
///     results = client.search("dark matter", rows=10)
#[pyclass(name = "AdsClient")]
struct PyAdsClient {
    client: AdsClient,
...
impl PyAdsClient {
    ...
    ///
    /// If `token` is None, reads from the `ADS_API_TOKEN` environment variable.
    #[new]
    #[pyo3(signature = (token=None))]
    fn new(token: Option<String>) -> PyResult<Self> {
        let client = match token {
            Some(t) => AdsClient::new(t),
            None => AdsClient::from_env().map_err(to_py_err)?,
        };
    ...
    /// Set a custom base URL (e.g., for SciX or testing).
    ...
    fn __repr__(&self) -> String {
        format!("AdsClient(base_url='{}')", self.client.base_url)
    }
}
...
/// Python module for the NASA ADS / SciX API client.
///
/// Usage:
///     import ads_client
...
///     results = client.search("dark matter", rows=10)
#[pymodule]
#[pyo3(name = "ads_client")]
pub fn init_module(m: &Bound<'_, PyModule>) -> PyResult<()> {
    // Wrapper types
    m.add_class::<PyAdsClient>()?;
