...
//!
//! Implements the JSON-RPC 2.0 protocol over stdio, exposing ADS API tools
...
pub async fn run_server(client: AdsClient) -> crate::error::Result<()> {
    let stdin = io::stdin();
    let stdout = io::stdout();

    for line in stdin.lock().lines() {
        let line = line.map_err(|e| AdsError::Config(format!("stdin error: {}", e)))?;
        ...
        let request: Value = match serde_json::from_str(&line) {
            ...
            Err(e) => {
                ...
                writeln!(stdout.lock(), "{}", error_response)
                    .map_err(|e| AdsError::Config(format!("stdout error: {}", e)))?;
                continue;
            }
        };
        ...
        stdout
            .lock()
            .flush()
            .map_err(|e| AdsError::Config(format!("stdout flush error: {}", e)))?;
    }
...
fn handle_resource_read(id: &Value, params: &Value) -> Value {
    ...
    let content = match uri {
        "ads://fields" => FIELDS_REFERENCE.to_string(),
        "ads://syntax" => SYNTAX_REFERENCE.to_string(),
        _ => {
            return json!({
                "jsonrpc": "2.0",
                "id": id,
...
async fn handle_tool_call(client: &AdsClient, id: &Value, params: &Value) -> Value {
    let tool_name = params["name"].as_str().unwrap_or("");
    let args = &params["arguments"];

    let result = match tool_name {
        "ads_search" => tool_search(client, args).await,
        "ads_bigquery" => tool_bigquery(client, args).await,
        "ads_export" => tool_export(client, args).await,
        "ads_metrics" => tool_metrics(client, args).await,
        "ads_library" => tool_library(client, args).await,
        "ads_library_documents" => tool_library_documents(client, args).await,
        "ads_citation_helper" => tool_citation_helper(client, args).await,
        "ads_network" => tool_network(client, args).await,
        "ads_object_search" => tool_object_search(client, args).await,
        "ads_resolve_reference" => tool_resolve_reference(client, args).await,
        "ads_resolve_links" => tool_resolve_links(client, args).await,
        _ => Err(AdsError::Config(format!("Unknown tool: {}", tool_name))),
    };
...
async fn tool_search(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let query = args["query"]
        .as_str()
        .ok_or_else(|| AdsError::InvalidQuery("'query' parameter required".into()))?;
    ...
    let results = client
        .search_with_options(query, fl, sort_val.as_ref(), rows, 0)
        .await?;

    Ok(format_search_results(&results))
}

async fn tool_bigquery(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let bibcodes: Vec<&str> = args["bibcodes"]
        .as_array()
        .ok_or_else(|| AdsError::InvalidQuery("'bibcodes' array required".into()))?
        .iter()
...
async fn tool_export(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let bibcodes: Vec<&str> = args["bibcodes"]
        .as_array()
        .ok_or_else(|| AdsError::InvalidQuery("'bibcodes' array required".into()))?
        .iter()
...
async fn tool_metrics(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let bibcodes: Vec<&str> = args["bibcodes"]
        .as_array()
        .ok_or_else(|| AdsError::InvalidQuery("'bibcodes' array required".into()))?
        .iter()
    ...
    serde_json::to_string_pretty(&metrics).map_err(|e| AdsError::Parse(e.to_string()))
}

async fn tool_library(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let action = args["action"]
        .as_str()
        .ok_or_else(|| AdsError::InvalidQuery("'action' parameter required".into()))?;

    match action {
        "list" => {
            let libs = client.list_libraries().await?;
            serde_json::to_string_pretty(&libs).map_err(|e| AdsError::Parse(e.to_string()))
        }
        "get" => {
            let id = args["id"]
                .as_str()
                .ok_or_else(|| AdsError::InvalidQuery("'id' required for get".into()))?;
            let lib = client.get_library(id).await?;
            serde_json::to_string_pretty(&lib).map_err(|e| AdsError::Parse(e.to_string()))
        }
        "create" => {
            let name = args["name"]
                .as_str()
                .ok_or_else(|| AdsError::InvalidQuery("'name' required for create".into()))?;
        ...
        "delete" => {
            let id = args["id"]
                .as_str()
                .ok_or_else(|| AdsError::InvalidQuery("'id' required for delete".into()))?;
        ...
        "edit" => {
            let id = args["id"]
                .as_str()
                .ok_or_else(|| AdsError::InvalidQuery("'id' required for edit".into()))?;
        ...
        _ => Err(AdsError::InvalidQuery(format!(
...
async fn tool_library_documents(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let action = args["action"]
        .as_str()
        .ok_or_else(|| AdsError::InvalidQuery("'action' parameter required".into()))?;
    let library_id = args["library_id"]
        .as_str()
        .ok_or_else(|| AdsError::InvalidQuery("'library_id' required".into()))?;
...
async fn tool_citation_helper(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let bibcodes: Vec<&str> = args["bibcodes"]
        .as_array()
        .ok_or_else(|| AdsError::InvalidQuery("'bibcodes' array required".into()))?
        .iter()
    ...
    serde_json::to_string_pretty(&result).map_err(|e| AdsError::Parse(e.to_string()))
}

async fn tool_network(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let bibcodes: Vec<&str> = args["bibcodes"]
        .as_array()
        .ok_or_else(|| AdsError::InvalidQuery("'bibcodes' array required".into()))?
        .iter()
...
async fn tool_object_search(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let objects: Vec<&str> = args["objects"]
        .as_array()
        .ok_or_else(|| AdsError::InvalidQuery("'objects' array required".into()))?
        .iter()
        .filter_map(|v| v.as_str())
        .collect();
...
async fn tool_resolve_reference(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let references: Vec<&str> = args["references"]
        .as_array()
        .ok_or_else(|| AdsError::InvalidQuery("'references' array required".into()))?
        .iter()
        .filter_map(|v| v.as_str())
        .collect();
...
async fn tool_resolve_links(client: &AdsClient, args: &Value) -> Result<String, AdsError> {
    let bibcode = args["bibcode"]
        .as_str()
        .ok_or_else(|| AdsError::InvalidQuery("'bibcode' required".into()))?;
...
fn format_search_results(results: &crate::types::SearchResponse) -> String {
...
fn tool_definitions() -> Value {
    json!([
        {
            "name": "ads_search",
            "description": "Search the NASA ADS database using ADS query syntax. Supports field queries (author, title, abstract, year, etc.), boolean operators, and functional operators (citations(), references(), similar()).",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "ads_library",
            "description": "Manage ADS personal libraries (list, get, create, edit, delete).",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "action": { "type": "string", "enum": ["list", "get", "create", "edit", "delete"] },
                    "id": { "type": "string", "description": "Library ID (for get/edit/delete)" },
        ...
        {
            "name": "ads_library_documents",
            "description": "Add or remove documents from an ADS library.",
            "inputSchema": {
                "type": "object",
                "properties": {
                ...
                "required": ["action", "library_id", "bibcodes"]
            }
        },
        {
            "name": "ads_citation_helper",
        ...
        {
            "name": "ads_resolve_reference",
            "description": "Resolve free-text references to ADS bibcodes (e.g., 'Einstein 1905 Annalen der Physik 17 891').",
            "inputSchema": {
                "type": "object",
                "properties": {
...
const FIELDS_REFERENCE: &str = r#"ADS Searchable Fields
=====================
...
const SYNTAX_REFERENCE: &str = r#"ADS Query Syntax Guide
======================
