//! CLI binary for the ADS client.
//!
//! Usage: ads search "dark matter" --rows 10

#[cfg(feature = "cli")]
mod cli {
    use ads_client::{AdsClient, ExportFormat, Sort, SortDirection};
    use clap::{Parser, Subcommand};

    #[derive(Parser)]
    #[command(name = "ads", about = "NASA ADS / SciX API client", version)]
    struct Cli {
        /// ADS API token (overrides ADS_API_TOKEN env var)
    ...
    fn make_client(token: Option<String>) -> ads_client::error::Result<AdsClient> {
        match token {
            Some(t) => Ok(AdsClient::new(t)),
            None => AdsClient::from_env(),
        }
    }
    ...
    fn print_papers_table(papers: &[ads_client::Paper]) {
    ...
    pub async fn run() -> ads_client::error::Result<()> {
        let cli = Cli::parse();
        ...
        match cli.command {
            Commands::Search {
                ...
                let fields_str = fields.as_deref().unwrap_or(ads_client::parse::DEFAULT_SEARCH_FIELDS);
                let results = client
