...
if [ "$BUILD_X86" = "1" ]; then
    ...
    cp "$BUILD_DIR/$MACOS_TARGET/release/libimpart_core.a" \
        "$MACOS_UNIVERSAL_DIR/libimpart_core.a"
fi
...
cargo run --release -p uniffi-bindgen -- generate \
    --library "$BUILD_DIR/$MACOS_TARGET/release/libimpart_core.a" \
    --language swift \
...
HEADERS_DIR="$FRAMEWORK_DIR/headers/impart_coreFFI"
mkdir -p "$HEADERS_DIR"
cp "$FRAMEWORK_DIR/generated/impart_coreFFI.h" "$HEADERS_DIR/"

cat > "$HEADERS_DIR/module.modulemap" << 'MODULEMAP'
module impart_coreFFI {
...
for dir in "$FRAMEWORK_DIR/$XCFRAMEWORK_NAME.xcframework"/*/Headers; do
    rm -f "$dir"/*/impart_core.swift 2>/dev/null || true
    rm -f "$dir/impart_core.swift" 2>/dev/null || true
    echo "  Cleaned $dir"
done
