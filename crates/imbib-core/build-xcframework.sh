#!/bin/bash
# Build script for imbib-core Rust library
...
if [ "$BUILD_IOS" = "1" ]; then
    ...
    if [ "$BUILD_X86" = "1" ]; then
        ...
        cp "$BUILD_DIR/$IOS_SIM_TARGET/release/libimbib_core.a" \
            "$IOS_SIM_UNIVERSAL_DIR/libimbib_core.a"
    fi
fi
...
cargo run --release -p uniffi-bindgen -- generate \
    --library "$BUILD_DIR/$MACOS_TARGET/release/libimbib_core.a" \
    --language swift \
...
impress_sync_file "$FRAMEWORK_DIR/generated/imbib_core.swift" "$FRAMEWORK_DIR/imbib_core.swift"
...
if [ -d "$APP_BINDINGS_DIR" ]; then
    impress_sync_file "$FRAMEWORK_DIR/generated/imbib_core.swift" "$APP_BINDINGS_DIR/imbib_core.swift"
...
echo "Swift bindings: $FRAMEWORK_DIR/imbib_core.swift"
echo ""
echo "To use in your Swift package, add the XCFramework as a binary target"
echo "and copy imbib_core.swift to your sources."
