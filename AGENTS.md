...
- **Regression prevention**: When optimizing or adding new code paths, check the "Critical Invariants" section in the relevant app's AGENTS.md for constraints the old code enforced implicitly. When fixing bugs, document the violated invariant there.
...
Use `mutableSetValue(forKey:)` for to-many relationship mutations, not direct property assignment. See [imbib AGENTS.md](apps/imbib/AGENTS.md) for details.
...
### Debugging Workflow for Codex Sessions
...
See `apps/*/AGENTS.md` for detailed app documentation:
- [imbib AGENTS.md](apps/imbib/AGENTS.md) - Bibliography manager specifics
