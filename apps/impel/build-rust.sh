#!/bin/bash
# Build script for impel Rust dependencies
#
# This script builds the impel-core Rust library and generates:
...
RUST_CRATE="$WORKSPACE_ROOT/crates/impel-core"
OUTPUT_DIR="$SCRIPT_DIR/Packages/ImpelCore"
FRAMEWORK_NAME="ImpelCoreFFI"

echo "Building impel-core ($BUILD_TYPE)..."
...
if [ ! -f "$RUST_CRATE/Cargo.toml" ]; then
    echo "Error: impel-core not found at $RUST_CRATE"
    exit 1
fi
...
cargo build $CARGO_FLAGS --target aarch64-apple-darwin --features native

echo "Building for x86_64-apple-darwin..."
cargo build $CARGO_FLAGS --target x86_64-apple-darwin --features native
...
AARCH64_LIB="$TARGETS_DIR/aarch64-apple-darwin/$BUILD_DIR/libimpel_core.a"
X86_64_LIB="$TARGETS_DIR/x86_64-apple-darwin/$BUILD_DIR/libimpel_core.a"
UNIVERSAL_DIR="$TARGETS_DIR/universal-macos-$BUILD_DIR"
UNIVERSAL_LIB="$UNIVERSAL_DIR/libimpel_core.a"
...
BINDINGS_DIR="$OUTPUT_DIR/Sources/ImpelCore/Generated"
...
cargo run --features native --bin uniffi-bindgen generate \
    ...
    cat > "$BINDINGS_DIR/impel_core.swift" << 'EOF'
// Placeholder Swift bindings
// Run uniffi-bindgen to generate real bindings
//
// cargo install uniffi_bindgen
// uniffi-bindgen generate --library libimpel_core.a --language swift --out-dir .

import Foundation

// Re-export types from ImpelCore.swift
...
if [ -f "$BINDINGS_DIR/impel_coreFFI.h" ]; then
    cp "$BINDINGS_DIR/impel_coreFFI.h" "$MACOS_FRAMEWORK_DIR/Headers/impel_core.h"
else
    # Create minimal header
    cat > "$MACOS_FRAMEWORK_DIR/Headers/impel_core.h" << 'EOF'
// impel_core.h - C header for impel-core FFI
...
<dict>
    ...
    <string>com.impress.impel.core</string>
