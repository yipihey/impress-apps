...
public struct PDFExtractedMetadata: Sendable {
    ...
    public var bestTitle: String? {
        if let title, Self.isPlausibleTitle(title) {
            return title
        }
    ...
    /// as junk from publisher templates or tooling. Exposed static so it
    ...
    static func isPlausibleTitle(_ candidate: String) -> Bool {
        ...
        // Explicit prefixes
        let junkPrefixes = ["pii:", "doi:", "microsoft word -", "untitled"]
        for prefix in junkPrefixes where lower.hasPrefix(prefix) {
            return false
        }

        // Common tooling defaults (exact match, case-insensitive)
        let junkExact: Set<String> = [
            "document1", "document", "manuscript", "paper", "article",
            "preprint", "draft", "untitled 1", "untitled document"
        ]
        if junkExact.contains(lower) { return false }

        // Bare PII shape: YYYY-YYYY(YY)NNNNNN-N or YYYY-YYYY...
        let piiRegex = #"^\d{4}-\d{4,6}(\([0-9A-Z]{1,6}\))?[-0-9A-Z]*$"#
        if trimmed.range(of: piiRegex, options: .regularExpression) != nil {
            return false
        }

        // Bare DOI
        if trimmed.range(of: #"^10\.\d{3,}/[^\s]+$"#, options: .regularExpression) != nil {
            return false
        }

        // All digits / all punctuation / all non-letters
