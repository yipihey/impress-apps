...
//
//  Imbib-side value type for `citation-usage@1.0.0` records. Written
//  by imprint's `CitationUsageTracker`; read by imbib's
//  `CitationUsageReader`.
//
//  A separate copy from imprint's `ImprintCore.CitationUsageRecord`
//  exists here because PublicationManagerCore does not link
//  ImprintCore — the two packages share a schema, not a type.
//

import Foundation
import ImpressRustCore

/// One record of "manuscript section X cites paper Y".
...
extension CitationUsageRecord {
    public init?(row: SharedItemRow) {
        ...
        // equality and imprint's ImprintStoreAdapter writes it bare. See
        // schema-refs.json.
