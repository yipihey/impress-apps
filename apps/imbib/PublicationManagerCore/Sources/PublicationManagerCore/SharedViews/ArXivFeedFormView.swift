...
public enum SearchFormMode: Equatable {
    ...
    /// Library feed (from + button or context menu on a library)
    /// - Stored in the specified library
    /// - Does NOT feed to inbox (feedsToInbox = false)
    /// - Auto-refreshes on schedule
    case libraryFeed(UUID, String)  // (libraryID, libraryName)
    ...
    /// Whether this mode creates an auto-refreshing feed
    public var isFeedMode: Bool {
        switch self {
        case .inboxFeed, .libraryFeed: return true
        case .librarySmartSearch, .explorationSearch: return false
        }
    }
...
public struct ArXivFeedFormView: View {
    ...
    private var headerTitle: String {
        switch mode {
        case .inboxFeed, .libraryFeed: return "Create arXiv Feed"
        default: return "arXiv Category Search"
        }
    }
    private var headerIcon: String {
        switch mode {
        case .inboxFeed, .libraryFeed: return "antenna.radiowaves.left.and.right"
    ...
    private var headerSubtitle: String {
        ...
        case .libraryFeed(_, let name): return "Auto-refreshing feed in \(name)"
    ...
    private func createFeed() {
        ...
        Task {
            do {
                ...
                case .libraryFeed(let libraryID, _):
                    smartSearch = RustStoreAdapter.shared.createLibraryFeed(name: effectiveFeedName, query: query, sourceIDs: ["arxiv"], libraryID: libraryID, maxResults: maxResultsParam, refreshIntervalSeconds: 3600, saveTargetID: saveTargetID)
                ...
                if mode.isFeedMode { await executeInitialFetch(smartSearch) }
                await MainActor.run {
    ...
    private func executeInitialFetch(_ smartSearch: SmartSearch) async {
        ...
        do {
            let fetchedCount = try await fetchService.fetchForFeed(smartSearchID: smartSearch.id)
            Logger.viewModels.infoCapture("Initial feed fetch complete: \(fetchedCount) papers added", category: "feed")
        } catch {
...
public struct ArXivFeedFormView: View {
    ...
    private func executeInitialFetch(_ smartSearch: SmartSearch) async {
        ...
        do { let c = try await fetchService.fetchForFeed(smartSearchID: smartSearch.id); Logger.viewModels.infoCapture("Initial feed fetch: \(c) papers", category: "feed") } catch { Logger.viewModels.errorCapture("Initial feed fetch failed: \(error.localizedDescription)", category: "feed") }
    }
