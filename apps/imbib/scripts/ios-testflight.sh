...
# Builds and uploads imbib iOS app to TestFlight for beta distribution
#
# Usage: ./scripts/ios-testflight.sh [version]
#        ./scripts/ios-testflight.sh --setup    # Store API credentials in Keychain
#
# Example: ./scripts/ios-testflight.sh v1.2.0-beta1
#
# Prerequisites:
# - Apple Silicon Mac (for iOS builds)
# - Xcode with iOS distribution certificate
# - xcodegen installed (brew install xcodegen)
# - Rust toolchain with aarch64-apple-ios target
# - App Store Connect API key (created at https://appstoreconnect.apple.com/access/api)
#
...
KEYCHAIN_SERVICE_RELEASE="imbib-release"  # Reuse team-id from release credentials

# ============================================================================
# Keychain Setup Mode
# ============================================================================
if [[ "$1" == "--setup" ]]; then
    echo "=== TestFlight Credential Setup ==="
    echo ""
    ...
    echo "(The Team ID is shared between release and TestFlight scripts)"
    echo ""
    echo "You can now run: ./scripts/ios-testflight.sh v1.0.0-beta1"
    exit 0
fi
...
echo -e "${GREEN}=== iOS TestFlight Build ===${NC}"
echo -e "Version:      ${YELLOW}$VERSION${NC}"
echo -e "Build:        ${YELLOW}$BUILD_NUMBER${NC}"
echo -e "Target:       ${YELLOW}aarch64-apple-ios (arm64)${NC}"
echo ""
...
if [ ${#MISSING_CREDS[@]} -gt 0 ]; then
    ...
    echo "  ./scripts/ios-testflight.sh --setup"
    echo ""
...
# ============================================================================
# Step 1: Build Rust for iOS
# ============================================================================
step_start "1/7" "Building Rust library (iOS arm64)..."
...
# Build macOS target (needed for uniffi-bindgen and combined XCFramework)
...
# Update build number using PlistBuddy on the generated project
PLIST_PATH="$IMBIB_DIR/imbib-iOS/Resources/Info.plist"
...
# ============================================================================
# Step 5: Archive iOS app
# ============================================================================
step_start "5/7" "Archiving iOS app..."
...
# ============================================================================
step_start "6/7" "Exporting IPA for App Store..."
...
IPA_PATH="$BUILD_DIR/export/imbib.ipa"
if [ ! -f "$IPA_PATH" ]; then
...
# ============================================================================
step_start "7/7" "Uploading to TestFlight..."
...
echo "  2. Add build to TestFlight testing groups"
echo "  3. Install via TestFlight on test devices"
echo ""
