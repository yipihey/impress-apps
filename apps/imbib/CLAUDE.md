...
**A figure's artifact lives in the STORE's blob root, and a figure surface re-reads on `.structural`.** (2026-09-25.) The image under a `figure` row's `data_hash` is in `<workspace>/content` — the directory `SharedStore.manuscriptProjectBlobRoot()` names, beside `impress.sqlite` in the suite group — written only by implore through `implore_core::figure_artifact` (`ImploreStoreAdapter.storeFigure` is the one write). It used to go to `~/.local/share/impress/content`, which inside the sandbox is each app's own CONTAINER home, so impress's `plot` pane could never read an artifact implore wrote: the same class of bug as the library container above. `FigureStoreReader.contentData` keeps the old per-app directory as a read fallback only. Separately, a write from ANOTHER process reaches this one only as `.structural` with no ids (Darwin note → `StoreMutationObserver`), so `FigureDetailPane` and `LayoutPlotPaneView` re-read on it through `FigureArtifactRefresh`, comparing the row first so an unrelated structural event neither re-renders nor logs. A blob is removed only by `SharedStore.release_blob`, which refuses while any live record of any kind names the digest (edit-history op rows do not count).
