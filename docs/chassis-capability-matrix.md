...
| Deferred to P9 | retire `RemarkableSyncManager` / `RemarkableSyncScheduler` / `RemarkableStatusSection` / `RemarkableConflictView` + `conflictResolution`, `AnnotationConverter`, `EInkAnnotation` / normalizer, the duplicated `RemarkableSettingsView` sections (still the local-network / cloud connect sheet), dead notifications; docs (`eink-devices.md`, ADR-025), keyboard-grammar rows; the sidebar "On reMarkable" node; iOS Info/Notes tabs do not render the new sections yet (macOS chromes only, per the Stage 5b split) |
