...
* **W3's, open:** only PUBLICATION rows of the tree's `list` pane are drag
  sources (`PublicationDragPayload`, the payload the chassis list row already
  wrote, now spelled once); figure and manuscript rows are not, so a figure
  cannot be dropped on a figure folder from the tree yet.
