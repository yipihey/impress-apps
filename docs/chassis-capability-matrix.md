...
|---|---|---|---|
| publication | ✅ the legacy menu (Open PDF, reMarkable, Copy/Cut/Copy Cite Key, Read, Star, Flag ›, Add Tag, Add to Library ›, Add to SciX Library ›, Remove from All Collections, Dismiss, Delete) | ✅ `PublicationDragPayload` (W3) | Star → `is_starred` 1, ★ on the row, menu then reads "Unstar"; Unstar → 0. Flag › Blue → `flag_color = blue`, stripe, Any Flag badge 2 → 3; Clear Flag → NULL. Add Tag → `ai` added (the pane asks with the triage "New Tag…" prompt; the legacy list opens its inline tag field) — removed through `triage-service_remove-tag`, because the publication menu (legacy and pane) has no remove-tag item and Undo did not reach it |
