...
| `section(.inbox)` | ✅ Add Feed / New Collection | ➖ | ➖ | ✅ reorder | ✅ pubs, expl. search→feed | ✅ | **Tree (W3):** same row, same menu; select → `set-query` the `inbox` named query with `$library` = the Inbox — read AND unread since W5, as the legacy list (it filtered to unread before: 57 rows where the legacy list showed 68) |
| `section(.libraries)` | ✅ New Library | ➖ | ➖ | ✅ reorder | ➖ | ➖ | **Tree (W3):** ✅ menu → New Library, live |
...
| `section(.figures)` | ✅ New Folder | ➖ | ➖ | ✅ reorder | ✅ folder→root | ➖ | Stage 2-B; **implore-only surface** — excluded by every other preset's `visibleSections` (imbib's became explicit 2026-07-27). The pragmatic appID gate stays as belt-and-braces for any shell that leaves `visibleSections` nil — since ADR-0022 G8 it is an owner SET, `{implore, impress}` (`AppShellConfiguration.facetOwnerAppIDs`), not an `appID ==` test: with equality the impress preset could permit the section and the sidebar would still drop it  **Layout tree (L8 W2), Mac-verified 2026-09-23:** `implore_default()` declares this section as three real panes; `/api/layout/tree` on 23123 shows `outline`+`list`+`info` over `figure` with NO `legacy` tile; Tier B is 8/8 against 23123; a `select` on the list pane put a `figure` id on channel 1 and the detail pane logged `pane 3 info: figure detail for <id>`, so the `info` pane reaches this kind's own detail view. **With a real row:** a figure seeded through implore's own `POST /api/figures` (which mirrors it into the store as `figure`) rendered its title, format and dates in the tree's detail pane, then was deleted from both the library and the store. The sidebar-NODE capabilities in this row (context menu / rename / delete / drag / drop / counts) are still the legacy route's; the tree's navigator pane is W3. |
| `section(.mail)` | ❌ none (no folder CRUD — IMAP owns folder lifecycle; Stage-2-A2 follow-up) | ➖ | ➖ | ✅ reorder | ➖ | ➖ | Stage 2-A; **impart-only surface** — same double gate as Figures (`visibleSections` everywhere else + `shouldShowSection` owner set `{impart, impress}`)  **Layout tree (L8 W2):** `impart_default()` already declares this section as `outline`+`list`+`info` over `q::mail()`, and the `info` pane now dispatches to `MessageDetailPane` instead of the publication-only `DetailView`. **Mac-verified 2026-09-23** on an impart built from this branch plus the `DirectoryArtifact.swift` duplicate-declaration fix (PR #50, which `main` still needs — impart does not compile without it): `/api/layout/tree` on 23122 shows `outline`+`list`+`info` over `message` with NO `legacy` tile; Tier B is 8/8; a live `chat-message` row selected on the list pane logged `pane 3 info: message detail for <id>` and rendered its sender and date. The sidebar-NODE capabilities in this row are still the legacy route's; the tree's navigator pane is W3. |
| `section(.agents)` | ❌ none (no task creation — the kernel schedules tasks; commands stay on impel's HTTP path) | ➖ | ➖ | ✅ reorder | ➖ | ➖ | Stage 2-C; **impel-only surface** — same double gate as Figures/Mail (`visibleSections` everywhere else + `shouldShowSection` owner set `{impel, impress}`)  **Layout tree (L8 W2), Mac-verified 2026-09-23:** `impel_default()` declares this section as three real panes; `/api/layout/tree` on 23124 shows `outline`+`list`+`info` over `task` with NO `legacy` tile; Tier B is 8/8 against 23124; a `select` on the list pane put a `task` id on channel 1 and the detail pane logged `pane 3 info: task detail for <id>`, so the `info` pane reaches this kind's own detail view. **With real rows:** a live `task@1.0.0` row rendered its state, assignee and latest run; an `info` pane over `agent-run` added by a `split` verb rendered a live `agent-run@1.0.0` row (`pane 5 info: agentRun detail for <id>`). The sidebar-NODE capabilities in this row (context menu / rename / delete / drag / drop / counts) are still the legacy route's; the tree's navigator pane is W3. |
| `section(.search)` | ✅ Show Hidden Forms | ➖ | ➖ | ✅ reorder | ➖ | ➖ | imbib only |
| `section(.flagged)` | ➖ | ➖ | ➖ | ✅ reorder | ➖ | ✅ | counts = pubs (imbib) / manuscripts (imprint). **Tree (W3):** its rows select `set-query … flag(color)` |
...
| `agentTasksAll` | ❌ none | ➖ kernel-owned | ➖ kernel-owned | ➖ | ➖ | ✅ `countItems` task@1.0.0 total | Stage 2-C fixed row; capabilities `.readOnly`; per-state smart children (queued/running/waiting_review/completed/failed/cancelled) as tree children, shown only when non-empty **Tree (W3), impel live:** select → `set-query` over `task` (the preset's own list query, so 0 verbs at launch); after Runs it also re-points the detail pane back to `task` |
| `agentTaskState` | ❌ none | ➖ | ➖ | ➖ | ➖ | ✅ `countItems` payloadEq state | Stage 2-C; smart child of Tasks; capabilities `.readOnly` **Tree (W3), impel live:** a scoped `legacy` pane (agents route, that state) — a task's lifecycle is `payload.state`, which `Filter::Status` does not read, so a query would list nothing under a badge of 1,738 |
| `agentRunsAll` | ❌ none | ➖ runs are immutable provenance | ➖ | ➖ | ➖ | ✅ `countItems` agent-run@1.0.0 total | Stage 2-C fixed row; capabilities `.readOnly` **Tree (W3), impel live:** select → `set-query` over `agent-run` + a detail `set-pane` to `item: agent-run`; a run clicked in the list → `pane 3 info: agentRun detail for …` |
...
| `searchForm` | ✅ Hide | ➖ | ➖ | ✅ reorder | ➖ | ➖ | **Tree (W3):** select → a scoped `legacy` pane (MATERIALIZE_FIRST `search`), live |
| `flagColor` | ❌ none | ➖ | ➖ | ✅ reorder | ➖ | ✅ | **Tree (W3):** counts = the legacy chassis root (Red 2), select → 2 rows, live |
...
| manuscript typst/latex | Info, Source, Preview (compiled PDF). In the layout tree Source is the session-bearing `source` view kind and Preview the `pdf` view kind over manuscripts — the same two views (`ManuscriptSourceTab`, `ManuscriptPreviewContent`), one per pane; see § View kinds |
| manuscript markdown | Info, Source, Preview (MarkdownUI, live) — the same two pane kinds in the tree |
| manuscript plaintext | Info, Source — `source` in the tree (a `pdf` pane over it shows "Nothing compiled yet") |
| figure with data_hash | Info, View (pdf tab relabeled; `FigureArtifactView` draws the CAS artifact — implore stores a PNG for every figure it writes; NSImage/UIImage also decode JPEG/TIFF/PDF, and NSImage SVG. A missing blob, SVG on iOS, or an undecodable format each get a hint naming why) |
...
the one field the preset algebra already expresses — the shipped presets'
`sectionBindings`, now `impress_layout_service::section_bindings`, pinned to
the named queries — and removed `usesLayoutTree` with the flag. What stays is
Swift-side for a reason each:

* `appID` — the key every Rust table is read by.
* `visibleSections` — read by the legacy sidebar (`ImbibSidebarViewModel`,
  which the tree's `outline` pane hosts) and by iOS's sidebar, which has no
  tree. Rust already knows each app's sections (`outline_sections`,
  `every_visible_section_is_accounted_for` pins the transcription); making
  the Swift field read that table is the next move, and it changes what iOS
  reads, so it is not done in passing.
* `defaultSection` / `defaultDetailTab` — the sidebar's launch selection
  and the legacy detail host's first tab; the tree's own launch arrangement
  is the preset.
* `recordKinds` (`RecordKindRegistry`) — holds view factories and closures;
  not data.
* `sectionBindings` as an initializer PARAMETER — a shell that is not
  shipped (the Litmus proofs that a new kind can join the chassis) binds a
  kind Rust's table cannot know.
* `auxiliaryRoutes`, `openOverrides`, `customSurfaces`, `presentableKinds` —
  host facts: which non-record routes, open behaviours, app-target views and
  renderable kinds this build has. None is a query; `customSurfaces` holds
  views.
...
| `Files/PDFManager.swift` (`AttachmentManager`), `Files/PDFDataValidator.swift`, `Chassis/PDFBrowser/PDFDownloadLedger.swift` | ✅ both (ledger: the macOS browser) | a paper's files are filed under the paper's OWN library (its store parent) whatever library the caller had in view, and `resolveURL` finds a file an older build misfiled under another library (only when its size matches the record) — the Info tab's browser import used the active library and made an APS PDF invisible to Open / Show in Finder / Delete (2026-09-11). PDF bytes enter the library only if PDFKit opens them and every announced byte arrived: the browser capture, `importAttachment(data:)` and `GET /api/papers/{citeKey}/pdf` (422 for a damaged copy) all ask `PDFDataValidator`, because a truncated download still starts with `%PDF`; the browser tracks downloads per entry, since one shared slot let two downloads of the same PDF read each other's temp file. A linked-file add, delete or materialization emits `.itemsMutated(kind: .attachment, ids: [owning publication])` — never a bare `.structural`, which every per-publication surface filters out by id, so a deleted attachment stayed on screen until the user switched papers and back (2026-09-11). `RustStoreAdapter.deleteLinkedFile` is the delete; `DELETE /api/papers/{citeKey}/files/{id}` is the same call for agents. The retired `.attachmentDidChange` notification had two posters that disagreed about the payload (`object` vs `userInfo`), so the store's own adds reached neither the PDF nor the Notes tab |
...
import half in `tests/eink_import.rs`). Nothing is re-sent silently
(`stale`, `removed_on_device`, `unmarked` are explicit states with "send
again").

**A folder the tablet lacks (2026-09-08).** Confirmed a second time by
reading the tablet's own bundle on this firmware: it issues exactly one
`GET /documents/…` and one `POST /upload` (a single `file` field, no
parent) — no folder can be created, renamed or moved by anyone but the
user, on the device. Waiting for that was the design, and in practice it
stopped every paper of a library whose name (`ULDM State & Coherence —
references`) nobody wants to type on a tablet. So the default is now
`file_in_nearest_folder`: the paper goes into the deepest folder on its
path that does exist, keeping `desired_path` on the mirror row, and the
Info section says where it belongs. Never above the root folder — with no
`imbib` on the tablet the paper still waits, and the row names the single
folder to create. The whole-tablet walk already follows a document the
user drags, so moving it into place clears `desired_path` by itself. The
...
| `imbib-eink-service_eink-status` | Settings › E-Ink Devices header: devices, counts (queued / on tablet / awaiting PDF / awaiting folder / filed in a parent folder / stale / failed), which device puts markers on rows, last sync |
| `imbib-eink-service_eink-devices` / `eink-configure-device` / `eink-remove-device` | the device card: mode (`all` \| `individual`), root folder, "mirror library and collection folders", "file in the nearest folder that exists" (`file_in_nearest_folder`), upload format, import toggles |
...
| `imbib-eink-service_eink-note-source-error` | how the app's PDF fetch for a marked paper went, written onto the `awaiting_source` row (`error` absent clears it). Only something that can download knows — the engine never can, so without it the row says nothing but "Awaiting PDF" |
...
### A manuscript's papers, in imbib's window (ADR-0032, 2026-09-12)
...
manuscript) hold every file except the entry, whose text stays in
`manuscript.body_content` — so every existing manuscript is already a one-file
project and nothing migrates. `impress_core::manuscript_project` is the one
writer (deterministic ids from paths, text inline and Automerge-backed through
the generalised `collab.rs`, binaries in the workspace CAS
`impress_core::blobs`); `imprint_core::project` scans LaTeX/Typst/Markdown
references and derives the build graph; `imprint-project-service` is the
store-direct verb surface — the CLI and MCP work with the app closed. Tier A:
`crates/impress-core/tests/manuscript_project.rs`,
`crates/imprint-core/tests/project_graph.rs`,
`crates/imprint-service/tests/project_service.rs`, selftest
`project.tree_roundtrip` / `project.graph_diagnostics` /
`project.import_export_snapshot` / `project.build_records`.

| Capability | MCP tool | Status | Evidence |
|---|---|---|---|
| the whole project in one read | `imprint-project-service_project-tree` | ✅ headless | entry path/hash, every file row (role, kind, format, hash, size, derivation, build/bib specs), targets (the implicit `main` when none are declared), `project_version` |
| one file's text (binary → temp path) | `imprint-project-service_project-file` | ✅ headless | the entry path returns the body; a blob not in this workspace's CAS is an honest `ok: false`, never an empty file |
| add / replace a file | `imprint-project-service_project-put-file` | ✅ headless | `content` (text) or `file_path` (bytes from disk); role from the extension unless given; refuses the entry path, `..`, `main` as a role, and watched-folder manuscripts (ADR-0023 D4) |
| delete / move a file | `imprint-project-service_project-delete-file`, `_project-move-file` | ✅ headless | a move is a new row (ids derive from paths) and outputs that named the old path as their source follow it; refuses to overwrite |
| declare the entry | `imprint-project-service_project-set-entry` | ✅ headless | Editorial, Durable op on the manuscript row; refuses a path that is a file row |
| declare targets | `imprint-project-service_project-set-targets` | ✅ headless | `[{id, name?, entry?, engine?, output_kind?, args?}]`; engines typst \| tectonic \| pdflatex \| xelatex \| lualatex \| latexmk \| markdown \| none; unknown engine refused here, not at build time |
| a projected bibliography | `imprint-project-service_project-set-bibliography` | ✅ headless (projection resolves in P2) | `{"kind":"cited"}` / `collection` / `library` / `keys` on a `.bib` row |
| how a figure is made | `imprint-project-service_project-set-figure-build` | ✅ headless (runners land in P4/P5) | `{runner, outputs[], inputs[], args{}}`; outputs and inputs must stay inside the tree |
| the derived build graph | `imprint-project-service_project-graph` | ✅ headless | edges with kind + line, unresolved references as `error` diagnostics naming file + line, include cycles, cite keys across the tree, figure steps in run order with staleness (`derived_from_hash` vs the step's input hash), reachability, unreferenced files, the target's bibliographies |
| the outline of the whole tree | `imprint-project-service_project-outline` | ✅ headless (P2) | every included file's sections spliced in where it is included, in reading order, with global `order_index` and the SAME ids the one-file outline derives (a one-file tree is byte-identical to `sections::extract`) |
| every citation in the tree | `imprint-project-service_project-citations` | ✅ headless (P2) | usages with the file each sits in (reading order first), plus the distinct keys — what the `cited` bibliography projects |
| compile a Typst target from the store | `imprint-project-service_project-compile` | ✅ headless (P2; Typst only until P4) | `imprint_core::project::typst::TreeFileResolver` serves the tree from memory — no directory, so it works on iOS and for the editor's preview; projected `.bib` rows are resolved through `imprint-service::project_bib::StoreBibliographyResolver` (raw BibTeX when the `imbib/bibliography-entry` row has it, synthesized from fields otherwise, missing keys as `missing-reference` warnings plus a placeholder entry so the document still compiles; the one-file convention holds — a citing Typst/Markdown tree without its own `.bib` gets the implicit `bibliography.bib` `cited` projection and an appended `#bibliography(...)`, `imprint_core::project::implicit_bibliography`); the PDF (or per-page SVGs) is written under `<cache>/impress/imprint/project-compile/<manuscript>/<target>/`; diagnostics name the FILE and line a span points into; `entry_override` is the live buffer. Not a recorded build — `project-build` (P4) is |
| a directory becomes a project | `imprint-project-service_project-import-directory` | ✅ headless (P3) | `imprint_core::project::import` walks the directory (build residue — `.aux`, `.log`, `_minted*`, `.git`… — skipped and reported), classifies by extension and then by use, and guesses the entry transparently (`entry_reason`; `entry` overrides). Into an existing manuscript the directory's entry becomes the body THROUGH the Automerge document (the old text stays reachable at its heads) and the format follows the entry; without `manuscript_id` a new manuscript is created titled `title` or the directory's name. Additive; refuses watched-folder manuscripts |
...
| the Build inspector (imprint) | — | ✅ GUI (P4) | `ProjectBuildSidePanel` + File ▸ Build Manuscript (⌥⌘B): target picker from `targets_json`, shell-steps toggle, the report (steps, diagnostics by file, outputs with Reveal/Open, log), recorded builds; `ManuscriptProjectModel.build` runs `project_build_tree` (imprint-core FFI, `ProcessRunnerHost` in-process) and records the row through `SharedStore.manuscriptProjectRecordBuild/FinishBuild` |
| previews of projects (imprint) | — | ✅ GUI (P2/P4) | `CompileInputs.projectFiles` — a Typst project previews through `compileTypstTree` (memory), a LaTeX project through `compileLaTeXTree` (the engine's materialised directory, shell steps never); a one-file manuscript keeps the single-source paths untouched |
| a folder becomes a manuscript (imprint) | — | ✅ GUI (P3) | File ▸ Import Folder as Manuscript… (`project_import_directory_plan` FFI decides; rows through the one writer; the entry through the document) and Files ▸ Import folder… into the open manuscript (refuses another grammar) |
| a new figure of any kind | `imprint-project-service_project-new-figure` | ✅ headless (P5) | `imprint_core::project::figures` — a starter of `veusz` (a document Veusz edits), `lilaq` (a Typst figure lilook edits; `@preview/lilaq:0.6.0` vendored), `typst`, `implore` (a `PlotSpec` turned into lilaq Typst), `impress-plot` (the native inspector's spec, `.plot.json` in the HTTP shape), `script` — as a `figure-source` row with the build spec that renders it to `<stem>.svg`; the kind is detected from the name and, for `.plot.json`, the spec's shape (`spec_kind_of`) |
...
**Status: the tree is the only root of every chassis app (plan wave 6 W5).**
`ChassisRootView` renders `LayoutTreeHost` for impress, impel, implore,
impart and imprint's main window. **Where the host lives (plan wave 6 W6):**
the generic host — `LayoutModel`, `LayoutController`, `LayoutTreeView`
(`LayoutTreeHost` / `LayoutWindowView` / the containers), `PaneSessionRegistry`,
`ViewKindRegistry` + `PaneContext`, and the `placeholder` and `surface` view
kinds (`LayoutSurfacePaneView`) — is the kit package `packages/ImpressLayout`,
which depends on ImpressRustCore, ImpressSurface, ImpressKeyboard and
ImpressLogging only (`scripts/check-kit-packages.sh`). Everything domain stays
in `apps/imbib/PublicationManagerCore/Sources/PublicationManagerCore/Chassis/Layout/`:
the `outline`, `list`, `info`, `legacy`, `pdf`, `notes`, `bibtex` and `source`
factories and their views, `LayoutPaneRowMapper`, the suite-aware surface hooks
(`LayoutSurfaceHooks.swift`) and `LayoutController+Automation` (ImpressAutomation
is not kit-grade). `ChassisViewKinds.registerIfNeeded()`, called from
`ChassisRootView.init`, registers them into `ViewKindRegistry.builtin`; the host
also hands the kit its store and automation hook through `LayoutHostServices`.
A kind nobody registered renders `placeholder` — `apps/kit-demo` (throwaway)
draws a tree of them plus one surface with nothing but the kit.
There is no switch: the `impress.layoutTree.enabled` defaults key,
`LayoutTreeFlag` and `AppShellConfiguration.usesLayoutTree` that gated it
...
* **No tree rendering → 409**, saying why (in a chassis app: the window has
  not opened its tree yet; in imbib: its window has none) and naming the
  headless path. Not a 200 with an empty body: that is what `/api/layout`
  did, below.
...
nothing" failure ADR-0032 documents for the two cross-app citation
channels.
...
| `pdf` | ✅ rendered — L8 W4, Mac-verified 2026-09-24 | `PDFTab` unchanged, in `LayoutPublicationTabPaneView` (the paper resolved like `info`: `single_item`, else the `item` binding), inside `publicationDetailLifecycle`, padded by `layoutToolbarBand`. **Over manuscripts (pass B):** `LayoutManuscriptPreviewPaneView` — the detail pane's Preview tab, moved verbatim into `ManuscriptPreviewContent` and shown by both, over the same `ManuscriptEditorSession` the `source` pane edits (imprint's Writing: typing beside it recompiles it); a file-backed manuscript says it is not compiled here | ➖ | Logs `pane N pdf: publication <id>` (`pane N pdf: manuscript <id> preview` over manuscripts). impress, a `pdf` pane split beside `info` over the detail query: a list click moved `pdf`/`notes`/`bibtex`/`info` to the same id in the same millisecond; the Banik & Sikivie PDF rendered (1/33); a paper with no identifiers showed PDFTab's own "No PDF". A pane over another kind says "Detail Unavailable" by name. **Fixed on the way (both hosts):** an auto-download finishing after a paper switch put the old paper's PDF in the pane (PDFTab and NotesTab now drop a completion for a paper they left). **Library PDFs open in every host (2026-09-24).** They were "not found" outside imbib: the files lived under `<Application Support>/imbib`, which under the sandbox is each app's own container, so impress looked in `com.impress.impress`'s (empty) container — no sandbox denial, it never asked for imbib's. Library files now live in the suite group (`LibraryFilesLocation.sharedRoot` = `<group>/workspace/imbib/Libraries/<id>/…`, beside `impress.sqlite`) and imbib copies its private ones across on launch (`LibraryFilesMigration`: verified, originals kept); see imbib CLAUDE.md. Re-proven in impress: the Banik & Sikivie PDF (444,020 B) rendered in a `pdf` pane split beside `info`, loaded from the group path. Tier B `layout.reading_pdf_pane` |
| `notes` | ✅ rendered — L8 W4, Mac-verified 2026-09-24 | `NotesTab` unchanged (PDF beside the notes panel), same pane view | ➖ | Logs `pane N notes: publication <id>`. Three-point trace through the pane: Key Findings typed → the store's `note` became `---\nKey Findings: …\n---\n\nN` (label-keyed front matter over the freeform, via `PublicationNotesDocument`) → re-selected, the pane re-parsed it; unchecking wrote back `N` byte-exact. `hjkl` typed into the field reached it. **Pre-existing, not fixed:** `NotesPanel` reloads on any `itemsMutated` for its paper and resets the visible annotation fields, and a click into a just-enabled empty field produces such an event, so the field vanishes (its guard covers only the freeform editor) |
| `bibtex` | ✅ rendered — L8 W4, Mac-verified 2026-09-24 | `BibTeXTab` unchanged, same pane view | ➖ | Logs `pane N bibtex: publication <id>`; showed the selected entry and followed each selection with `pdf`/`notes`/`info` |
| `source` | ✅ rendered + **session-bearing** — L8 W4 pass B, Mac-verified in imprint 2026-09-24 | `ManuscriptSourceTab` unchanged (editor, preview split, compile strip) in `LayoutSourcePaneView`, the manuscript resolved like `info`; its TEXT is the manuscript's `ManuscriptEditorSession` (`ManuscriptSessionRegistry`, the one save/compile path), its EDITOR is the pane's `SourcePaneSession` | ✅ `SourcePaneSession` in `PaneSessionRegistry<SourcePaneSession>` keyed by `PaneSpec.session`; it owns a `TypstEditorHost` — scroll view, `TypstTextView`, the one coordinator, one `UndoManager` per manuscript shown | **Rust decides the id** (`impress_layout::sessions`, 13 tests + Tier A `source-pane-sessions`): every `source` pane holds one (`ViewKindId::SESSION_BEARING`), no two share one, a split's NEW pane gets a fresh one even from a copied spec, swap/move/resize/close/set_view_kind keep them, `set_pane` keeps the replaced pane's, a preset or saved layout re-applied adopts the live session of the pane in the same ROLE, and a first load assigns and saves (so a second process reads the same ids). The representable builds the editor once through the Source tab's own `makeEditor`, MOVES it into each new pane container, and on dismantle detaches it only if it is still its own (SwiftUI dismantled the old host before building the new one here) — no `.id` anywhere. **Proof (imprint, throwaway manuscripts, since deleted):** typed `W4B hjkl typed`, then a split (editor re-parented, `mounted (mount 2)`), a second split that wrapped it again (mount 3), a swap (mount 4) and preset 1 re-applied (mount 5 after re-selecting): the same `ObjectIdentifier(0x0000000805a89900)` each time, ⌘Z undid the typing and ⇧⌘Z redid it every time (store body read back), ONE source session opened in the whole run, `session-ec2e2242…` unchanged in `/api/layout/tree`. Switching the pane between two manuscripts kept a history each (⌘Z in one never touched the other). **D6 liveness:** a `commit-manuscript-body` from `impress` (the CLI, another process) landed in place — `took an external change … in place: 0 chars at 134 replaced by 35`, no remount — including against a stale base while a keystroke was still inside the 200 ms debounce (Automerge forked from one head and merged both; the caret stayed with its text). **Delete:** `discarded editor session …` then `source session …: abandoned manuscript … (deleted) — was on screen; pending save cancelled, undo history dropped, nothing written`; the row stayed gone. A manuscript with `external_source` gets a read-only snapshot and no session (`ManuscriptEditorSessionPolicy`, moved to PMC; imprint's `WatchedManuscriptGuard` calls it). Logs: `pane N source: session S manuscript M editor E`, `source session …: editor E mounted / detached / shows manuscript / took an external change`. Tier B `layout.source_pane_session` (imprint + impress 11/11). **Gaps:** a narrow pane clips the Source tab's columns (their min widths sum past the pane); a preset re-application also clears the channels, so the editor comes back on the next selection |
| `plot` | ✅ rendered — Mac-verified in implore 2026-09-24 | `FigureArtifactView` in `LayoutPlotPaneView` (PMC, `Chassis/Layout/`): the figure's rendered artifact (`data_hash` → the content store), which is `FigureDetailPane`'s View tab body moved into one public view that the tab and the pane both call. Resolved like `info` (`single_item`, else the `item` binding), padded by `layoutToolbarBand`, re-read on a store event for the figure, or on `.structural` (how another process's write arrives) when the row changed | ➖ | **Why this reuse point:** a store `figure` carries no plot spec. implore draws from its own session and mirrors the RESULT into the content store, so the artifact is the figure's plot for every app except implore. `renderPlotSvg` / the surface plot views draw a `plot-spec@1.0.0`, which no figure references, and implore's canvas is implore-core, which PMC does not link. Logs `pane N plot: figure <id>` (plus ` (no rendered artifact)` when there is none). **Named empty states, never the placeholder:** a pane over another kind ("This pane lists “publication”…"), a value that is not an id, an id that is another record ("“F0C378B5…” is a “bibliography-entry”, not a figure", logged `pane N plot: bibliography-entry <id> is not a figure`), no row, and no selection (`noRowSelection(kind: .figure)`). `LayoutPlotTargetTests` (6, FFI-free). **Proof (implore, 23123, this branch):** a `plot` pane split beside `info` (a copy of the detail spec, `view_kind: plot`, no role) over a figure seeded through implore's `POST /api/figures`; one list `select` logged `pane 5 plot: figure C563336D… (no rendered artifact)` and `pane 3 info: figure detail for C563336D…` in the same millisecond, and the pane showed the View tab's own "No renderable artifact". The same figure was then given a PNG (imprint's `renderPlotSvg` through `POST /api/plot/render`, rasterized, written to implore's content store under its sha256, `data_hash` set with the store FFI's additive `upsert_item`), and the pane drew the plot beside the Info tab, which now lists the hash. The pane came back from the persisted tree on relaunch (`pane 5 plot: figure C563336D…` at window open). Kindless `item(<publication id>)` query: "Plot Unavailable" naming the kind. Figure, CAS file and store row removed afterwards (0 figure rows). **Fixed on the way:** the artifact sat in a two-axis `ScrollView` at its natural size, so a 1384 pt plot showed only its top-left corner in the pane AND in the View tab; it now fits the space it has and is never enlarged past its natural size. **Artifacts (figure-artifacts branch, 2026-09-25):** every implore figure write stores its artifact. `implore_core::figure_artifact` renders the figure's view state (`type`, columns, size, and `series` / `spec` / `svg` data) to SVG and resvg-rasterises it to a 2× PNG in `<workspace>/content` (the store's blob root — the old `~/.local/share/impress/content` was each app's SANDBOX home, so impress could never read implore's bytes); `ImploreStoreAdapter.storeFigure` is the one path for `POST`/`PATCH /api/figures`, `create-figure` (over HTTP), export and the plot viewer's Save as Figure (⌥⌘S). An edit releases the superseded blob; `DELETE` removes the row, its exports, and the blob unless any live row of any kind still names it (`SharedStore.release_blob`). `/api/figures/{id}/export` (GET, and POST for `export-figure`) writes `<workspace>/exports/figures/<id>.<png|svg>`. **Proof (implore + impress, that branch):** `POST /api/figures` with inline series → `data_hash` set, 1280×840 PNG in the app-group `content/`; impress `split` + `plot` pane logged `pane 6 plot: figure CB0E6DAB… artifact 7a9a69b2128a` and drew it; implore's View tab drew it; a `PATCH` in implore redrew impress's open pane 0.8 s later with no navigation (`… artifact 5cb093ff28b1`) and released the old blob; `create-figure` / `export-figure` through impress-mcp returned a figure and a file path; `DELETE` left 0 rows, 0 blobs, 0 exports. **Data (create-figure-data branch, 2026-09-25):** `create-figure` takes at most one of `series` (`[{label?, x, y}]`, drawn in `plot_type`'s style), `spec` (a whole implore `PlotSpec`; only `series[].x`/`y` required, the rest defaults) or `svg` (finished SVG); none of them keeps labelled empty axes. `implore_core::figure_artifact::validate_figure_data` checks them before anything is sent (caps: 32 series, 50 000 points, 2 MB of SVG, 4096 points a side) and refuses naming the argument and index; the answer is `ok` + figure + `artifact` + `drawn_from`, or `ok: false` + `error` (MCP `isError`). The data reaches `POST /api/figures` under the keys `copyArtifactData` already reads, so the stored artifact is the plot through the one renderer. The `impress` CLI now installs implore's HTTP backend for implore verbs, as impress-mcp does. implore binds `-httpAutomationPort N` (launch argument) and `/api/status` reports it. **Proof (implore 23181 + impress 23182, that branch):** `impress create-figure … --series run 1 --series run 2` → `ok`, `drawn_from: series`, 800×600 artifact `46d1c640de32`; `implore-service_create-figure` over impress-mcp stdio with one scatter series → `ok`, artifact `6b0445e1e1a6`, and a call giving `series` + `svg` → `isError: true`, "give at most one of series, spec and svg; got series and svg"; impress `split` (tile 3, kindless `item(<figure id>)`) + `set-view-kind plot` logged `pane 19 plot: figure 9B2EF5CE… artifact 6b0445e1e1a6` and drew the six points, then `set-query` to the CLI figure (`… artifact 46d1c640de32`) drew both lines with a legend; implore's View tab (`view_state.tab = pdf`) drew each figure's data; `--spec` (log y, error bars) → 640×400 artifact, `--svg` → 300×200. All four deleted: 0 figure rows, 0 blobs, 0 exports. **Gaps:** there is no update verb, so data can change only through `PATCH /api/figures` over HTTP, where a new data key is added beside an old one (`svg` wins over `spec` over `series`) rather than replacing it; `create-figure` still requires `dataset_id` and `x` when inline data makes them labels; the `impress` CLI exits 0 on an `ok: false` answer; SVG artifacts show a hint on iOS (UIImage has no SVG decoder) |
| `console` | ✅ rendered — **the kit's own** (`packages/ImpressLayout`), Mac-verified in impress and implore 2026-09-24 | `LayoutConsolePaneView`: `ImpressLogging.ConsoleView` unchanged (the ⌘⇧C console), over the same `LogStore.shared`, embedded (no 600×300 window minimum), padded by `layoutToolbarBand` | ➖ | **Why the kit:** `ConsoleView` is ImpressLogging's, already a kit-grade dependency of ImpressLayout (kit-manifest.md), and knows no record kind or store, so a host that registers nothing (`apps/kit-demo`) gets a real console. `ViewKindRegistry.builtin` is now placeholder + surface + console. **Scope = the console's own two controls, from `view_state`** (opaque to the tree, owned by the view kind): `{"search": "layout", "levels": ["warning", "error"]}` seeds the search field (which matches message OR category, so a category name is the category filter) and the level toggles; anything else is ignored, a malformed value is the whole log (`LayoutConsoleScopeTests`, 6). The query is ignored: a log is not store items. Logs `pane N console: <app> log, <scope>` (`all`, or `search 'layout', levels info,warning,error`). **Proof (impress, this branch):** `split` of the detail pane + `set-view-kind console` on the new tile, no new verb: `pane 6 console: impress log, all`; maximized, the pane showed the live log, including the split, the set-view-kind and its own line. Tier B `layout.console_pane` (split with a scoped `view_state`, the spec read back unchanged, the pane's own line awaited): impress **13/13**, implore **13/13**. **Gap:** in a narrow column (≈500 pt) the console's toolbar squeezes its four level toggles to blank chips and the row columns clip — the toolbar is the window's; a field edited in the pane is not written back to `view_state` |
| `surface` | ✅ rendered — Mac-verified 2026-09-22/23 | `SurfaceView(tree:)` in `packages/ImpressSurface`, mapping a `RenderTree` node to SwiftUI one to one with no logic of its own (ADR-0033 D2, work package S7), hosted by `LayoutSurfacePaneView` (`packages/ImpressLayout` since W6; the kit's factory uses `SurfaceHooks.plain`, and PMC re-registers `surface` with `SurfaceHooks.chassis` — MarkdownUI, `renderPlotSvg`, record rows) | ➖ — a surface's working state is the `impress/ui/surface-state@1.0.0` row, not anything the pane itself holds | A pane of this kind renders a stored `impress/ui/surface@1.0.0` document: the pane's query is `item(id)` of the `surface` kind (registered in `impress-core`'s pane-query builtin manifest, same as every other kind). Reached by `surface_show` (composes ordinary `layout-service` verbs — split/take a role, set the query, set the view kind — so a surface pane is not a special case of the tree) and mirrored at `/api/surface/*`, per the `/api/layout/*` pattern above. Select → the surface's own `publish` action runs on the pane's channel, the same channel mechanism every pane uses; there is no rename/delete/drag/drop OF THE PANE — those act on the surface RECORD through `surface_update`/`surface_delete`, not on the tile. **Mac-verified (impress, driven from impress-mcp as a second process):** the pane appears in the running window with no HTTP to the app (D6 — needed the cross-process AND the in-process liveness fixes in `impress-store-ffi::layout`); j/k walk the focus ring; a slider drag is ONE `change` on release; the histogram renders through `renderPlotSvg`; `GET /api/surface`, `GET …/<id>/render`, `POST …/<id>/dispatch` answer on 23125 with the verbs' shapes; `surface_wait` returned `bins-chosen {"bins": 40}` when the button was clicked in the window; a table row's `publish` puts `publication: [id]` on channel 1 and the detail pane's `item` binds to it; an `open` effect dispatched over HTTP grew the tree 8 → 9 tiles at once. Enter on the highlighted button fires its `click` (one `kind click` event, watched in the log); Escape drops a slider's real focus back to the highlight (the knob's focus halo goes, the highlight box stays). A `list` of six papers renders as mail-style rows and grows with them. **Headless: everything through `surface_render`** — it returns the exact `RenderTree` (every `{{path}}` resolved, `focus_order` computed) that the Swift renderer would otherwise turn into pixels, so a surface's behaviour is Tier-A testable without opening the app |

### The outline sidebar (plan wave 6 W3, 2026-09-23)

The navigator pane is the app's sidebar for real. What each row does when
selected is decided in Rust (`crates/impress-layout-service/src/outline.rs`,
`outline_target` + `outline_verbs`, 15 tests) and reached from Swift through
two FFI free functions (`outline_sections_json`, `outline_row_verbs_json`);
Swift maps `ImbibTab` → `OutlineNode` (`Chassis/Layout/LayoutOutlineNode.swift`)
and applies the verbs it is given. Selecting a row never re-implements a
sidebar action: context menu, rename, delete, drag and drop are the same
`ImbibSidebarViewModel` code the legacy chassis sidebar runs, so a cell below that
says "same code" was proven in that host and is the same call here.

**What a selected row becomes** (`OutlineTarget`):

| Node (tab) | Tree does | Evidence (impress, 23125, live) |
|---|---|---|
| `library` | `select` library on channel 1 + `set-query` publications `parent = <id>` — which compiles to `HasParent OR ReferencedBy(Contains)` since W5 pass B, the legacy list's `in_library_predicate` (the manifest's `contains_members`: library → publication) | Tom's papers: badge 153, `pane 2 display: 153 rows`; channel 1 carries the library. A library holding a paper parented elsewhere by a `Contains` edge lists it too: see the Inbox row below (its only such paper today) and imbib-core `a_compiled_library_pane_matches_in_library_predicate` |
| `libraryCollection` / `inboxCollection` / `explorationCollection` | `select` collection on channel 1 + `set-query` publications `collection = <id>` | "State, Coherence…": badge 78, `pane 2 display: 78 rows`; throwaway collection: badge 1 → 1 row |
| `anyFlag` / `flagColor` | `set-query` of the shell's flagged kind + `flag(color)` | Red: badge 2, `pane 2 display: 2 rows` |
| `section(.inbox)` / `allInbox` | the `inbox` named query with `$library` = the Inbox library | **W5:** every Inbox paper, read and unread, as the legacy route lists them (`queryPublications(parentId:)`, `disableUnreadFilter`); the badge alone is unread. W3 recorded `pane 2 display: 57 rows` against the legacy 68 — the query filtered `read: false` then. Presets moved to shipped revision 2; a row still exactly revision 1 is upgraded (`ensure_shipped`), an edited one is left alone. Rust test `the_inbox_lists_read_and_unread_papers_like_the_chassis_list` runs both revisions over a store. Live (impress, W5 build, no flag): the stored list held revision 1 with the Inbox id bound (the outline had written it) — the launch selection now takes it (`is_superseded_list_query`) and logged `outline select (launch): … → query, 1 verb(s)`, a `set-query` with `"filters":[]`, then `pane 2 display: 67 rows` (was 57). Pass A stopped at 67 against the legacy 68: the legacy list is `queryPublications(parentId:)` = `HasParent OR ReferencedBy(Contains)` (`in_library_predicate`), and one Inbox paper (ab9c0da4, "Illuminating the Local Universe…", parented to Save) is in the Inbox only by a `Contains` edge. **Pass B (decided by Tom, 2026-09-24):** a `Parent` scope whose parent is a library (the kind manifest's `contains_members`, not a kind string in the lowering) compiles to that same Or; figure and mail folders stay `HasParent`. Live (impress, pass B build): `pane 2 display: 68 rows`, the legacy predicate over the store 68 (67 parented + ab9c0da4); after imbib's Friends feed imported 17 papers, `pane 2 display: 85 rows` against the predicate's 85 and imbib's own list (`rebuildRowData: 85 rows`) |
| `tag`, `watchedFolder` | `set-query` `tag(path)` over the shell's Tags kind (a watched folder's papers are its provenance tag) | Rust test `a_tag_row_is_a_query_even_though_the_tag_outline_is_not` |
| `allArtifacts`, `dismissed` | the `artifacts` / `dismissed` named query (`$dismissed_library` bound) | Rust tests |
| record rows: `figuresAll`, `mailAllInboxes`, `agentTasksAll`, `agentRunsAll`, `journalAll`, folders, flagged/tag of a kind | `set-query` over the kind; a folder by its membership (`figure`/`message` → `parent`, envelope children only — the legacy readers file by parent alone, so W5 pass B's library widening does not reach them; `manuscript` → `collection-subtree`, else `collection`); a detail `set-pane` when the list's kind changes, so `info` hears the new kind on the channel | Rust tests `folder_scopes_follow_the_membership_table`, `a_kind_change_repoints_the_detail_pane_…` |
| `searchForm`, feed forms | **legacy** `search` (MATERIALIZE_FIRST) | SciX Search clicked: `set-pane` → `legacy`, `view_state.section = search`; the pane shows ONLY the ADS Modern form under "search — hosted, not yet a query" |
| `scixLibrary`, `sharedLibrary`, `section(.reviewQueue)` | **legacy** `scixLibraries` / `sharedWithMe` / `reviewQueue` (MATERIALIZE_FIRST) | rows hidden by the sidebar's own content gates in this store (no SciX key readable, `sharedWithMe` always hidden, 0 pending reviews); the scoped pane itself was driven over HTTP — see the W3 session log |
| `inboxFeed` / `libraryFeed` / `explorationSearch`, `recent`, `artifactType`, `citedInManuscripts`, task-state rows, a host-keyed record scope, `recordDetail`, `auxiliary`, `customSurface` | **legacy**, scoped to that route, with the reason Rust gives (a smart search is not stored results; `recent` is a view history; an artifact TYPE is a schema ref; Cited in Manuscripts' query needs `$manuscript`; a task's lifecycle is `payload.state`, not `payload.status`) | Rust tests; each is recorded, not hidden |

A query row after a legacy one is a `set-pane` back to `list` (params, channel
and role kept, `view_state` cleared) — seen live: Red after SciX Search. The
chassis' launch selection retargets the list ONLY when the list is still on
the preset's query (`initial_selection_applies`): relaunching with the list on
Tom's papers left it there and logged `not applied`.

**The sidebar rows, re-proven in the tree** (impress, W3, CGEvents; "legacy" = the pre-W5 `TabContentView` root on the same build):

| Node kind | Context menu | Rename | Delete | Drag | Drop target | Counts | Select → list → detail |
|---|---|---|---|---|---|---|---|
| `section(.libraries)` | ✅ New Library / Add Watched Folder… | ➖ | ➖ | same code | ➖ | ➖ | ➖ header |
| `library` | ✅ Rename / New Collection / Add Feed… / Export… / Import… / Delete Library | same code (menu path proven on `libraryCollection`) | ✅ confirm alert, throwaway library deleted (`Libraries: 7`) | same code | ✅ (a paper dropped on a collection of it, below) | ✅ = legacy (2657, 2992, 533, 153★7, 25, 78) | ✅ 153 rows, `info` follows a list click (`pane 3 info: publication detail for …`) |
| `libraryCollection` | ✅ Rename / New Subcollection / Delete | ✅ menu → inline field → `renamed … → 'ZZ W3 outline probe'` | ✅ menu (`deleted … members unfiled: 0`) and ⌫ (`members unfiled: 1`) | ✅ subcollection dragged onto its library: `reparented … → root` | ✅ paper dragged from the tree's `list` pane: `filed 1/1 item(s)`, badge 0 → 1 | ✅ = legacy (78) | ✅ 78 rows |
| `flagColor` / `anyFlag` | ➖ none (as legacy) | ➖ | ➖ | same code | ➖ | ✅ = legacy (2, 2) | ✅ 2 rows |
| `searchForm` | same code (Hide) | ➖ | ➖ | same code | ➖ | ➖ | ✅ scoped legacy pane |

**Gaps seen, and which side of W3 they are on:**

* **Pre-existing (the legacy chassis sidebar on the same build does the same):** a
  newly created library or collection does not open its inline rename field
  (logged `Rename requested for node …`, no editor); the menu's Rename does.
* **Closed by W4 (2026-09-24):** figure and manuscript rows of the tree's
  `list` pane are drag sources too (`FigureDragPayload`,
  `ManuscriptDragPayload`, extracted from their lists) — see § List-pane rows.
* **Closed by W5:** deleting the SELECTED collection or library. The legacy
  sidebar does NOT fall back to the parent: `deleteFolder` /
  `deleteCollection` set its selection to nil, a deleted library leaves a
  selection that resolves to nothing, and the content column says "No
  Selection". Rust's `outline_cleared_verbs` matches that — an empty
  `select` on the navigator's channel and on the detail's kind, the list
  left on its now-empty query, no row chosen for the user. The outline pane
  calls it when the sidebar's selection goes to nil and when the routed
  library leaves `LibraryManager.libraries`. **Live (impress):** a throwaway "New Collection" in ChemistryKernels holding one paper, the paper open in `info`, Delete from the collection's menu → `outline cleared: {"id":"aca3277b…","node":"collection"} — the sidebar's selection went to nil → 2 verb(s), no row selected`, `select → version 53`, `select → version 54`, `pane 2 display: 0 rows`; the window showed "Nothing Here" | "No Selection" and channel 1 read `collection: [], publication: []`. A throwaway "New Library" selected with a paper on the channel, Delete Library through its confirmation → `outline cleared: {"id":"4fba819d…","node":"library"} — its library was deleted → 2 verb(s)`, channel 1 `library: [], publication: []`, libraries back to 7
* **Closed by W5:** beside a scoped legacy route the tree's `info` pane kept
  the last selection. A legacy row's verbs now end with an empty `select` of
  the detail pane's kind (`a_hosted_route_empties_the_detail_pane_of_whatever_kind_it_reads`).
  **Live (impress):** a read Inbox paper in `info` (`pane 3 info: publication detail for 9DCF6737…`), then SciX Search clicked → `outline select: {"form":"ads-modern","node":"search-form"} → legacy, 2 verb(s)`, `set-pane → version 42`, `select → version 43`; the `info` pane showed "No Selection". **Still open:** the hosted route's own list|detail
  split is cramped in a list-width column.

### List-pane rows (plan wave 6 W4, Mac-verified 2026-09-24)

A `list` pane row's context menu and drag are its kind's OWN list's, extracted
so both hosts call one definition: `PublicationRowContextMenu` +
`PublicationListActions.chassis` (from `PublicationListView` /
`UnifiedPublicationListWrapper`), `ManuscriptRowChrome` (from
`ManuscriptListWrapper` / `ManuscriptSectionView`), `FigureRowChrome` (from
`FigureListWrapper` / `FigureSectionView`); messages, tasks and agent runs show
the shared `TriageMenu.items` over `RecordTriageActions.storeBacked`, which is
all their own lists show. What a menu did to its list's own state is done to
the tree's: the selection is the channel's, the scope is read off the pane's
query (`LayoutPaneScope`, 7 tests). All proofs in impress (23125), each
mutation reverted.

| Row kind | Context menu | Drag | Evidence |
|---|---|---|---|
| publication | ✅ the legacy menu (Open PDF, reMarkable, Copy/Cut/Copy Cite Key, Read, Star, Flag ›, Add Tag, **Remove Tag ›** (2026-09-24), Add to Library ›, Add to SciX Library ›, Remove from All Collections, Dismiss, Delete) | ✅ `PublicationDragPayload` (W3) | Star → `is_starred` 1, ★ on the row, menu then reads "Unstar"; Unstar → 0. Flag › Blue → `flag_color = blue`, stripe, Any Flag badge 2 → 3; Clear Flag → NULL. Add Tag → `ai` added (the pane asks with the triage "New Tag…" prompt; the legacy list opens its inline tag field) — removed through `triage-service_remove-tag`, because the publication menu (legacy and pane) has no remove-tag item and Undo did not reach it. **Closed 2026-09-24:** Remove Tag › lists every tag the targets carry and removes the path from the rows carrying it (`PublicationTagRemoval`, over `RustStoreAdapter.removeTag` — the verb `addTag` pairs with; trace `removeTag:` / `removeTag save:` / `removeTag display (list | pane N):`, category `tags`). `onRemoveTag` now takes the tag PATH: it took a tag UUID, and all five handlers (this menu's hosts, the row chip menu, tag-delete mode, iOS) were empty TODOs. Undo did not reach Add Tag in impress because `UndoCoordinator.shared` had no undo manager outside imbib's `ContentView`, so `registerUndo` returned before registering; `ChassisRootView` now wires it (`.wireUndo`), and both tag verbs undo/redo from Edit ▸ Undo in both hosts. **Live 2026-09-24:** imbib's legacy list (GUPPY I, `ai/type/theoretical`): `removeTag save: … on 0 of 1 pub(s) (undo: registered)`, `removeTag display (list): 0 of 1`; ⌘Z → `1 of 1`, ⇧⌘Z → `0 of 1`. impress's tree list pane (Foster et al. 2018): `removeTag display (pane 2): 0 of 1`; the Edit menu read "Undo Remove Tag '…'", Edit ▸ Undo → `1 of 1`, Edit ▸ Redo → `0 of 1`, then Edit ▸ Undo walked back through Add Tag to no tags (op log: add_tag / remove_tag / `undo:`-reasoned inverses). Both papers left as found. **Closed 2026-09-24:** background feed imports no longer land on the window's undo stack — feed / group-feed / smart-search / watched-folder work runs in `UndoCoordinator.performAutomatic` (see apps/imbib/CLAUDE.md, Critical Invariants). **Live 2026-09-24 (imbib's legacy list):** Paper ▸ Move to Collection… on a paper at 18:48:58; the scheduled group-feed refresh 'Friends' then created 13 papers (18:49:12–15) and logged 17 `Undo: not registering … — automatic work (group feed refresh 'Friends')` (13 Import Paper, 2 Add to Collection, Mark as Unread, Add to Library); Edit ▸ Undo at 18:49:38 undid the MOVE (paper back in its library, collection empty), not an import. The row's triage verbs are also reachable from the Paper menu now, each observed by the list (they posted notifications nothing observed): Dismiss from Inbox ⇧⌘J (live: → Dismissed, Edit ▸ Undo → back), Move to Collection… ⌃⌘M (picker; live: library → A, A → B leaves A, one Undo/Redo covers the move), Add to Collection… ⌘L (live: joins B, stays in its library), Remove from Collection ⇧⌘L (live: 2 papers out of B) — on throwaway papers in a throwaway library, all deleted after |
| manuscript | ✅ = `ManuscriptListWrapper`: Open in imprint, Duplicate, Rename…, Star/Dismiss/Archive, Flag ›, Tags ›, Delete…, Remove from Folder (folder-scoped) | ✅ `ManuscriptDragPayload` | Duplicate → `Created manuscript AC123311…`; Rename… (the section's alert) → `renamed manuscript AC123311… → 'ZZ W4 pane probe'`; dragged onto a folder → `sidebar dropped 1 manuscript(s) into folder f99d09b2…`; in the folder-scoped list Remove from Folder → 0 rows; Delete… (the section's confirmation) → `delete manuscripts: AC123311…`, count back to 64 |
| figure | ✅ = `FigureListWrapper`: Open in Canvas, Star, Flag ›, Tags ›, Delete…, Remove from Folder (folder-scoped) | ✅ `FigureDragPayload` | a figure seeded through implore's `POST /api/figures`; a throwaway figure folder from the Figures header; drag → `sidebar dropped 1 figure(s) into folder ae6a8116…`, `parent_id` = folder; Remove from Folder → NULL; Delete… → `delete figures: DBE69FE6…` (store row gone), implore's entry removed with its own `DELETE`, folder deleted from its menu |
| task / agent-run | ✅ = `AgentRecordListWrapper` (Star, Flag ›, Tags ›) | ➖ (their list has none: no folder tree) | a task (3FCB6694…): Star → 1 → Unstar → 0; Flag › Red → Clear Flag → NULL; Tags › `ai/field/physics` added, then checked and removed from the same submenu |
| message | ✅ = `MessageListWrapper` (Star, Flag ›, Tags ›) | ➖ (their list has none) | same code path as task; not driven live |
...
Nothing in `packages/ImpressLayout` or PMC's `Chassis/Layout/` holds layout state (ADR-0019 D3 / ADR-0031
