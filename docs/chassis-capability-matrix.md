...
| `section(.inbox)` | ✅ Add Feed / New Collection | ➖ | ➖ | ✅ reorder | ✅ pubs, expl. search→feed | ✅ | **Tree (W3):** same row, same menu; select → `set-query` the `inbox` named query with `$library` = the Inbox (UNREAD only: 57 rows where the flag-off list shows 68) |
...
| `library` | ✅ full | ✅ | ✅ confirm | ✅ | ✅ pubs, collections | ✅ | **Tree (W3), live:** menu ✅, delete confirm ✅, counts = flag-off ✅, select → `select` + `set-query parent=<id>` → list → `info` ✅ |
...
| `flagColor` | ❌ none | ➖ | ➖ | ✅ reorder | ➖ | ✅ | **Tree (W3):** counts = flag-off (Red 2), select → 2 rows, live |
...
`ImbibSidebarViewModel` code the flag-off sidebar runs, so a cell below that
...
| `section(.inbox)` / `allInbox` | the `inbox` named query with `$library` = the Inbox library | launch: `set-query` → `pane 2 display: 57 rows`. **Differs from the flag-off list (68):** the named query is UNREAD in the Inbox (`q::inbox`, `read: false`), the legacy route lists every Inbox paper. The section has no badge, so no count disagrees; the list does |
...
**The sidebar rows, re-proven in the tree** (impress, flag on, CGEvents):
...
| `library` | ✅ Rename / New Collection / Add Feed… / Export… / Import… / Delete Library | same code (menu path proven on `libraryCollection`) | ✅ confirm alert, throwaway library deleted (`Libraries: 7`) | same code | ✅ (a paper dropped on a collection of it, below) | ✅ = flag-off (2657, 2992, 533, 153★7, 25, 78) | ✅ 153 rows, `info` follows a list click (`pane 3 info: publication detail for …`) |
| `libraryCollection` | ✅ Rename / New Subcollection / Delete | ✅ menu → inline field → `renamed … → 'ZZ W3 outline probe'` | ✅ menu (`deleted … members unfiled: 0`) and ⌫ (`members unfiled: 1`) | ✅ subcollection dragged onto its library: `reparented … → root` | ✅ paper dragged from the tree's `list` pane: `filed 1/1 item(s)`, badge 0 → 1 | ✅ = flag-off (78) | ✅ 78 rows |
| `flagColor` / `anyFlag` | ➖ none (as flag-off) | ➖ | ➖ | same code | ➖ | ✅ = flag-off (2, 2) | ✅ 2 rows |
| `searchForm` | same code (Hide) | ➖ | ➖ | same code | ➖ | ➖ | ✅ scoped legacy pane |

**Gaps seen, and which side of W3 they are on:**

* **Pre-existing (the flag-off sidebar on the same build does the same):** a
...
* **W3's, open:** deleting the SELECTED collection or library leaves the list
  pane on that id's (now empty) query — the view model's selection goes to
  nil and a nil selection sends no verb. The list says "Nothing Here", which
  is true; what the flag-off window shows after the same delete was not
  compared.
* **W3's, open:** while a scoped legacy route is up, the tree's `info` pane
  keeps the last selection beside it (the hosted route brings its own detail
  half), and the hosted route's own list|detail split is cramped in a
  list-width column.
