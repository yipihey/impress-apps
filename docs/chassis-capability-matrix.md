...
|---|---|---|
| `library` | `select` library on channel 1 + `set-query` publications `parent = <id>` | Tom's papers: badge 153, `pane 2 display: 153 rows`; channel 1 carries the library |
...
| record rows: `figuresAll`, `mailAllInboxes`, `agentTasksAll`, `agentRunsAll`, `journalAll`, folders, flagged/tag of a kind | `set-query` over the kind; a folder by its membership (`figure`/`message` → `parent`, `manuscript` → `collection-subtree`, else `collection`); a detail `set-pane` when the list's kind changes, so `info` hears the new kind on the channel | Rust tests `folder_scopes_follow_the_membership_table`, `a_kind_change_repoints_the_detail_pane_…` |
