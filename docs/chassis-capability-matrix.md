...
| `watchedFolder` (ADR-0023 W1, 2026-07-31) | ⏳ W2 (chassis supplies `offersRefresh` / `offersReauthorization`; the menu is the host's) | ⏳ W2 | ⏳ W2 | ➖ a watched folder is a feed, not a container | ➖ discovery is one-way (D4: the watcher never writes user files) | ✅ **conditionally** — `WatchedFolderRowState.badgeCount` returns nil unless `state.countIsTrustworthy && !countIsPartial` | **W1 ships the CHASSIS surface only; imbib's wiring is W2.** Not a new `ImbibSidebarNodeType` case and not a new `SidebarSectionType`: a folder row is `RecordSidebarScope.host(kind, key: "watched-folder.<uuid>")`, emitted through the existing `RecordSidebarSectionContent(nodes:)` seam, so no chassis enum grew a case (`WatchedFolderRowState.sidebarNode(kind:)`, `[WatchedFolderRowState].sidebarParentNode(kind:)`). macOS-only in v1 by DECLARED availability (`FolderWatchAvailability.livePlatforms == [.macOS]`, D6) — the row model, filters, batching and bookmark store are cross-platform and iOS renders `scanOnDemand` honestly. **Degraded volumes never render as "0 files"** (D6 risk register): the row always carries a `WatchedFolderState` (`live` / `fallback` / `scanOnDemand` / `inaccessible(bookmarkStale:)`) whose `label` the row shows verbatim, and the badge is suppressed rather than understated. **Note for W2:** macOS routes its sidebar through `ImbibSidebarViewModel`, which does NOT read `RecordSidebarBuilder` — wiring the macOS row is a W2 decision (a node case there, or the `customSurface` seam) |
