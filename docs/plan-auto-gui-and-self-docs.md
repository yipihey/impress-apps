...
|---|---|---|
| 467 `#[impress_method]`s across 20 crates | **433 verbs, 38 services, 16 crates** in the linked `full` inventory. A grep for `#[impress_method` finds 452 lines, 19 of them in doc comments, tests, `examples/echo_demo.rs` and `impress-mcp`'s own docs; the brief's number was a grep. `apps/impel-tui` is the 74th workspace member; 73 are under `crates/` | `McpToolDescriptor::iter()` with `full`; `cargo metadata` |
