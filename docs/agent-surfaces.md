...
binary that actually depends on the crate (ADR-0033 D4). That link happens in
exactly one place, `crates/impress-capabilities`, behind a feature named
after the capability. The scaffold's registration step adds that feature and
an `optional = true` dependency; if `impress-capabilities` does not yet have
`[features]`/`[dependencies]` sections to anchor on (it is being written by a
concurrent work package as this is written), the script prints the two lines
to add by hand instead of guessing where they'd go. Once linked, `{"verb":
"<name>-service_echo", "args": {...}}` is usable in any surface's `sources`
or in a `call` action, exactly like `surface-demo-service_series` in the
...
and `impress-cli`) and `kit` (store + layout + surface + surface-demo — the
capabilities the standalone cut ships without a second copy of an app's
domain core). A capability generated by this script is *not* added to `kit`
automatically; that is a deliberate per-capability decision (does this
capability belong in the standalone kit, or is it suite-specific?), made by
editing `impress-capabilities/Cargo.toml`'s `kit` feature list by hand.
