...
6. **The question to keep asking:** What *specific operations* work across item types that would not work without the unified protocol? If the answer is only "they are in the same database," the unification is not earning its complexity.
...
`canonical_id` (for future collaboration, where two stores refer to the same logical entity) is `Option<String>` holding a UUID from a shared namespace agreed among collaborators. It is not used in Phase 1.

### D2. The Item Struct is the Canonical Definition

All data across the impress suite is represented as `Item` values. There are no app-specific data model types at the persistence layer; apps define typed Swift wrappers over items (see D13).
...
pub struct Item {
    ...
    pub canonical_id: Option<String>, // Shared identity across collaborating stores (see D1)
...
The `modified` timestamp is a stored column in the `items` SQL table, set to `NOW()` on each `update()` call. It is not on the `Item` struct (it would be stale immediately after any operation); it is included in SQL sort descriptors via the field path `"modified"`.

The previous ADR proposed deriving `modified` from the most recent operation item. This was rejected: deriving it requires a join or subquery on every sort, which is expensive for large lists. A materialized column is updated as a side effect of each write and costs nothing on read.
...
**The anti-pattern:** accessing `payload["title"]` directly outside the adapter layer. Code that does this is brittle against schema evolution and obscures intent.

**The accessor pattern (Rust side):** each schema module exposes functions that take `&Item` and return typed values. In the Swift layer, each app defines typed Swift structs wrapping `Item` (see D13).
...
**Currently implemented variants** (as of this ADR, in `crates/impress-core/src/reference.rs`):

| Variant | Meaning | Typical use |
|---|---|---|
| `Cites` | Academic citation | Paper A cites paper B |
| `References` | General reference | Item links to another without implying citation |
...
**Target taxonomy variants not yet promoted to named variants:** `IsPartOf`, `HasVersion`, `Mentions`, `TriggeredBy`, `Exports`. These are additive — `Custom("is-part-of")` can be used until they are promoted. Domain-specific edges that appear in more than one place in the codebase should be promoted.

**Tradeoff:** A closed enum forces exhaustive match arms, which is valuable for correctness. The `Custom` escape hatch provides extensibility without losing that guarantee for known types.
...
Each schema's Rust module exposes typed accessor functions rather than requiring callers to use raw `payload` map access. This is the enforced discipline; scattered `payload["title"]` access in application code is an anti-pattern.

Example accessor pattern:
...
Callers write `bibliography::title(&item)`, not `item.payload["title"]`. The accessor is the schema contract. Raw `payload` access is permitted only inside these accessor functions and inside UniFFI adapter methods.

### D9. The Relational Store with Graph-Structured Relationships

The implementation is SQLite (via rusqlite, feature-gated under `features = ["sqlite"]`) with WAL mode. The SQL schema:
...
store_metadata     — key/value pairs for store-level config (origin_id, logical_clock, tag_namespace)
items_fts          — FTS5 virtual table (title, author_text, abstract_text, note)
```

**This is not a graph database.** Graph databases (Neo4j, TigerGraph) provide native graph storage engines, traversal query languages (Cypher, Gremlin), and index-free adjacency. This implementation is a relational store with a typed adjacency list table (`item_references`) and a `neighbors()` method that performs recursive SQL joins up to a caller-specified depth. Calling it a graph database would be false advertising.
...
- SQLite has 20+ years of reliability and is the only widely-deployed embedded database with broad platform support including the iOS sandbox.
...
### D10. App Architecture: Single Process, Shared SQLite File

Each impress app is a single macOS process. Apps do not share a process; they share a file. The shared store lives in the app group container. SQLite WAL mode allows multiple processes to read concurrently with one writer at a time.
...
The consequence of this model: there is no dedicated server process, no IPC protocol to define, and no network partition to handle. The shared state is a file. The tradeoff is that write throughput is limited to what SQLite can sustain (typically 10,000-100,000 simple writes per second on modern hardware), which is adequate for human-paced research workflows and moderate agent output. High-frequency agent logging at scale would require a dedicated write process and is deferred.

### D11. Event Notification Model: Darwin Notifications, Not a Ring Buffer

When a mutation is committed to the store, the `SqliteItemStore` emits an `ItemEvent` via an in-process `std::sync::mpsc` channel. The Swift `RustStoreAdapter` bridges this and posts a Darwin notification (via `CFNotificationCenter`) so that other apps observing the same store can refresh their view.

```
SqliteItemStore.update()
  -> mpsc::Sender<ItemEvent>       (in-process)
  -> RustStoreAdapter (Swift)
  -> CFNotificationCenter.post("com.impress.store.didMutate")
  -> Other app processes receive notification, query the store for current state
```
...
The previous ADR proposed a lock-free ring buffer in shared memory as the agent output channel. This is deferred. The ring buffer becomes necessary only when agent output exceeds what Darwin notification + poll can handle without visible latency. That threshold has not been reached.

**The startup render loop hazard:** background services that call `storeDidMutate` during the first ~90 seconds of app launch cause perpetual SwiftUI body re-evaluation (spinning beach ball). All background services that mutate the store must apply a startup grace period delay before their first work cycle. This is not optional. The `SmartSearchRefreshService` has a 60-second startup grace period; `InboxScheduler` has a 90-second startup delay. See CLAUDE.md for the exact mechanism and detection test.
...
Each Swift app defines typed structs that wrap `Item`. These structs are view models over the underlying `Item`; they are not persisted.

Example pattern (imbib's `Publication`):

```swift
// In imbib — wraps Item for the bibliography domain
struct Publication {
    let item: Item
...
- Localizes all string-literal field name usage to one place
...
Attention routing logic lives in app-specific middleware that subscribes to store events. This logic is different per app and per user configuration, and placing it in the core would make it impossible to customize without forking the library.
...
This query model directly replaces imbib's `PublicationSource` enum and `LibrarySortOrder` type:

- Library filter: `ItemQuery { predicates: [HasParent(libraryId)] }`
- Smart search: `ItemQuery { predicates: [Or([Contains("title", q), Contains("abstract", q)])] }`
- Flagged papers: `ItemQuery { predicates: [HasFlag(Some("red"))] }`

### D16. Item is the Universal Communication Channel Between Apps

The item is not just a storage format; it is the inter-app communication channel. When imbib adds a paper to a reading list, it writes an item. When an agent in impel produces a recommendation, it writes an item with `author_kind: Agent` and a reference to the target paper. When impart receives an email that should trigger a paper lookup, it writes a request item that impel observes.

Apps do not call each other's APIs. They write items that the other app is watching for. The coordination is through the store, not through XPC, URL schemes, or App Group notifications — except that Darwin notifications are used to signal "look at the store now."

This model has a concrete benefit: agent-produced output and human-produced content are in the same store, with the same query API, with the same provenance model. There is no separate "agent output log" that requires different tooling to inspect.
...
- **Provenance by design.** The operation log records every classification change with author, timestamp, and intent. Audit trails and undo are available without additional plumbing.
...
- **Schema evolution path.** The `SchemaRegistry` with inheritance allows adding specialized schemas without duplicating fields.

### What Becomes Harder

- **Migration cost.** Imbib's existing Core Data model must be migrated to `impress-core`. This is a one-time cost, but it is real. The migration must produce identical user-facing behavior.

- **Schema design is consequential.** Field naming choices in a schema affect all apps and all queries. A field named `"author_text"` in imbib's FTS extractor must match what impart's FTS extractor expects if cross-app search is to work.

- **Debugging the operation layer.** Classification changes produce two writes: the materialized update and the operation item. Debugging why an item's `is_read` is incorrect requires looking at both. The three-point trace pattern (mutation → save → display) in CLAUDE.md addresses this.

- **Write amplification.** Every user action on a classification field (marking read, adding a tag) produces two writes: the materialized update and the operation item. Acceptable at human interaction pace; requires review at high agent output rates.

- **SQLite contention under concurrent agents.** SQLite serializes writers. If multiple agent processes write to the shared store simultaneously, they queue. Not a problem for current usage (one app writing at a time); becomes a problem at agent scale. Deferred.

### What is Explicitly Deferred

- **Hot/warm/cold storage tiers.** Not needed at current scale. Revisit if imbib approaches 500,000+ items or agent logging produces more than ~10,000 items per day.

- **High-throughput agent output channel (ring buffer).** The Darwin notification + poll model is adequate for Phase 1-2.

- **Collaboration and `canonical_id` semantics.** The field exists on `Item` but its semantics are not defined in this ADR.

- **Schema versioning and migration.** Must be solved before Phase 1 ships.
...
2. **FTS extractor registration.** Currently, `SqliteItemStore` calls a hardcoded FTS extraction path. When Phase 2 (impart) introduces `chat-message` items, how does the store know what text to index? Possible: each `Schema` carries an optional `FtsExtractorId` that the store looks up in a registered extractor map.

3. **Cross-app write coordination.** If imbib and impart both have the store open, SQLite serializes their writes. At what point does write queue depth become visible latency to the user? No profiling has been done.

4. **Tombstones and deletion semantics.** `SqliteItemStore` has an `init_tombstones` function visible in the source but not described in this ADR. What is the tombstone model? Are deleted items recoverable? Is deletion cascaded to operation items (losing provenance of the deleted item)?

5. **The `neighbors()` depth limit.** The current signature allows any `depth: u32` but there is no enforced maximum. Deep traversal on a large graph will produce slow SQL. Should the store enforce a maximum depth (e.g., 5)? Should it return a partial result with a `truncated` flag?

6. **`ItemQuery` pagination stability.** `limit` and `offset` are supported but offset-based pagination is unstable if items are inserted between pages. For impart's message stream (append-only, high volume), cursor-based pagination (by `created` timestamp) would be more correct.

7. **The `subscribe()` contract.** Currently returns `Receiver<ItemEvent>` but the `SqliteItemStore` implementation emits events only to the in-process channel, not across processes. Cross-process subscribers receive a Darwin notification and must re-query. Should the `subscribe()` contract be clarified to reflect this limitation?

8. **Operation items and the schema registry.** Operations are stored as items with schema `impress/operation`, but this schema is not registered in `SchemaRegistry` in the current codebase. `registry.validate()` will reject operation items if called on them. Is the operation schema intended to be registered? If so, the definition needs to be added to `schemas/`.
...
- `docs/ADR-0001-unified-item-architecture.md` — superseded prior ADR (Proposed, 2026-02-08)
